...
REQUIRED_SOURCE_REPORT_ROLES: tuple[str, ...] = (
    ...
    "phase2ey_replay_bundle_report",
    "phase2ez_negative_control_report",
)

REQUIRED_REPRODUCTION_STEPS: tuple[str, ...] = (
    "phase2ew_replay_bundle_summary_cross_directory_replay",
    "phase2ex_replay_bundle_summary_replay_negative_controls",
...
def _bundle_artifact_entries(phase2ey: dict[str, Any]) -> list[dict[str, Any]]:
    manifest = _read_json(phase2ey.get("evidence", {}).get("bundle_manifest"))
...
def _reproduction_steps(
    *,
    ...
    phase2ex: dict[str, Any],
    phase2ey: dict[str, Any],
    phase2ew_report_json: str | Path,
    phase2ex_report_json: str | Path,
    phase2ey_report_json: str | Path,
    phase2ez_report_json: str | Path,
) -> list[dict[str, Any]]:
    return [
        {
            "step_id": "phase2ew_replay_bundle_summary_cross_directory_replay",
            "module": "reflexlm.cli.audit_phase2ew_replay_bundle_summary_cross_directory_replay",
            "inputs": {"phase2ev_report_json": phase2ew.get("evidence", {}).get("phase2ev_report_json")},
            "outputs": {
                "output_report_json": str(phase2ew_report_json),
                "output_dir": phase2ew.get("evidence", {}).get("replay_dir"),
            },
        },
        {
            "step_id": "phase2ex_replay_bundle_summary_replay_negative_controls",
            "module": "reflexlm.cli.audit_phase2ex_replay_bundle_summary_replay_negative_controls",
            "inputs": {"phase2ew_report_json": str(phase2ew_report_json)},
            "outputs": {
                "output_report_json": str(phase2ex_report_json),
                "output_dir": phase2ex.get("evidence", {}).get("negative_control_output_dir"),
            },
        },
        {
            "step_id": "phase2ey_replay_bundle_summary_replay_bundle",
            "module": "reflexlm.cli.audit_phase2ey_replay_bundle_summary_replay_bundle",
            "inputs": {"phase2ex_report_json": str(phase2ex_report_json)},
            "outputs": {
                "output_report_json": str(phase2ey_report_json),
                "output_dir": phase2ey.get("evidence", {}).get("bundle_dir"),
            },
        },
        {
            "step_id": "phase2ez_replay_bundle_summary_replay_bundle_negative_controls",
            "module": "reflexlm.cli.audit_phase2ez_replay_bundle_summary_replay_bundle_negative_controls",
            "inputs": {"phase2ey_report_json": str(phase2ey_report_json)},
            "outputs": {
                "output_report_json": str(phase2ez_report_json),
            },
        },
    ]


def validate_phase2fa_replay_bundle_summary_reproducibility_manifest(
    report: dict[str, Any],
) -> dict[str, Any]:
    ...
    checks = {
        "artifact_family_matches_phase2fa": (
            report.get("artifact_family")
            == "phase2fa_replay_bundle_summary_reproducibility_manifest"
        ),
        "top_level_phase2fa_passed": report.get("passed") is True,
        ...
        "source_phase2ey_and_phase2ez_passed": (
            report.get("source_summary", {}).get("phase2ey_passed") is True
            and report.get("source_summary", {}).get("phase2ez_passed") is True
        ),
        "source_phase2ey_validation_passed": (
            report.get("source_summary", {}).get("phase2ey_validation_passed") is True
        ),
        ...
        == report.get("source_summary", {}).get("phase2ey_copied_control_artifact_count"),
...
def audit_phase2fa_replay_bundle_summary_reproducibility_manifest(
    *,
    phase2ez_report_json: str | Path,
    output_manifest_json: str | Path,
    output_report_json: str | Path,
) -> dict[str, Any]:
    phase2ez = _read_json(phase2ez_report_json)
    phase2ey_report_json = phase2ez.get("evidence", {}).get("phase2ey_report_json")
    if not phase2ey_report_json:
        raise ValueError("Phase2FA requires Phase2EZ evidence.phase2ey_report_json")
    phase2ey = _read_json(phase2ey_report_json)
    phase2ex_report_json = phase2ey.get("evidence", {}).get("phase2ex_report_json")
    phase2ew_report_json = phase2ey.get("evidence", {}).get("phase2ew_report_json")
    if not phase2ex_report_json or not phase2ew_report_json:
        raise ValueError("Phase2FA requires Phase2EY source report evidence paths")
    phase2ex = _read_json(phase2ex_report_json)
    phase2ew = _read_json(phase2ew_report_json)
    phase2ey_validation = validate_phase2ey_replay_bundle_summary_replay_bundle(
        phase2ey
    )
    source_reports = [
        _file_entry(role="phase2ew_report", path=phase2ew_report_json, content_type="application/json"),
        _file_entry(
            role="phase2ex_negative_control_report",
            path=phase2ex_report_json,
            content_type="application/json",
        ),
        _file_entry(
            role="phase2ey_replay_bundle_report",
            path=phase2ey_report_json,
            content_type="application/json",
        ),
        _file_entry(
            role="phase2ez_negative_control_report",
            path=phase2ez_report_json,
            content_type="application/json",
        ),
    ]
    bundle_artifacts = _bundle_artifact_entries(phase2ey)
    reproduction_steps = _reproduction_steps(
        phase2ew=phase2ew,
        phase2ex=phase2ex,
        phase2ey=phase2ey,
        phase2ew_report_json=phase2ew_report_json,
        phase2ex_report_json=phase2ex_report_json,
        phase2ey_report_json=phase2ey_report_json,
        phase2ez_report_json=phase2ez_report_json,
    )
    manifest = {
        "artifact_family": "phase2fa_replay_bundle_summary_reproducibility_manifest",
    ...
    checks = {
        "source_phase2ez_passed": phase2ez.get("passed") is True,
        "source_phase2ey_passed": phase2ey.get("passed") is True,
        "source_phase2ey_validation_passed": phase2ey_validation.get("passed") is True,
        "source_phase2ez_negative_controls_complete": (
            phase2ez.get("metrics", {}).get("negative_control_count")
            == phase2ez.get("metrics", {}).get("negative_controls_failed")
        ),
        ...
        "control_artifact_count_preserved": _control_artifact_count(bundle_artifacts)
        == phase2ey.get("metrics", {}).get("copied_control_artifact_count"),
    ...
    report = {
        ...
        "metrics": {
            ...
            "control_artifact_count": _control_artifact_count(bundle_artifacts),
            "reproduction_step_count": len(reproduction_steps),
            "phase2ez_negative_control_count": phase2ez.get("metrics", {}).get(
                "negative_control_count"
            ),
            "phase2ez_negative_controls_failed": phase2ez.get("metrics", {}).get(
                "negative_controls_failed"
            ),
        },
        "source_summary": {
            "phase2ey_passed": phase2ey.get("passed") is True,
            "phase2ez_passed": phase2ez.get("passed") is True,
            "phase2ey_validation_passed": phase2ey_validation.get("passed") is True,
            "phase2ey_copied_control_artifact_count": phase2ey.get("metrics", {}).get(
        ...
        "supported_claims": [
            (
                "reproducibility manifest for the bounded Phase2EY replay bundle "
                "and Phase2EZ negative-control evidence"
            )
        ]
        if passed
        else [],
        ...
        "next_required_experiment": (
            ...
            if passed
            else "repair_phase2fa_replay_bundle_summary_reproducibility_manifest"
        ),
        "evidence": {
            "phase2ez_report_json": str(phase2ez_report_json),
            "phase2ey_report_json": str(phase2ey_report_json),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build Phase2FA replay bundle summary reproducibility manifest."
    )
    parser.add_argument("--phase2ez-report-json", required=True)
    ...
    report = audit_phase2fa_replay_bundle_summary_reproducibility_manifest(
        phase2ez_report_json=args.phase2ez_report_json,

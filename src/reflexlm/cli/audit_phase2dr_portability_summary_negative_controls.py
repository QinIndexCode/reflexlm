...
def _materialize_control_report(
    *,
    phase2dq_report: dict[str, Any],
    case_dir: Path,
) -> dict[str, Any]:
    control_report = deepcopy(phase2dq_report)
    source_markdown = _markdown_path(phase2dq_report)
    if not source_markdown.exists():
        raise ValueError("Phase2DR requires a readable Phase2DQ markdown summary")
    target_markdown = case_dir / "phase2dq_portability_summary.md"
...
def _mutate_source_summary_failed(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    report.setdefault("source_summary", {})["phase2do_passed"] = False
...
def _mutate_phase2do_validation_failed(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    report.setdefault("source_summary", {})["phase2do_validation_passed"] = False


def _mutate_negative_controls_incomplete(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    report.setdefault("source_summary", {})["phase2dp_negative_controls_failed"] = 0
...
def _mutate_markdown_missing_dimension(report: dict[str, Any], case_dir: Path) -> None:
    ...
    if not isinstance(rows, list) or not rows:
        raise ValueError("Phase2DR requires summary rows for markdown dimension control")
...
def _mutate_markdown_missing_boundary(report: dict[str, Any], case_dir: Path) -> None:
    ...
    text = text.replace("not an epoch-making architecture", "bounded architecture evidence")
...
CONTROL_SPECS: tuple[dict[str, Any], ...] = (
    {
        "control_id": "positive_control_original_phase2dq_summary",
        "mutation": None,
    ...
    },
    {
        ...
        "expected_failed_checks": ["source_phase2do_and_phase2dp_passed"],
    },
    {
    ...
    {
        "control_id": "negative_phase2do_validation_failed",
        "mutation": _mutate_phase2do_validation_failed,
        "expected_passed": False,
        "expected_failed_checks": ["source_phase2do_validation_passed"],
    },
    {
...
def _run_control(
    *,
    ...
    control_report = _materialize_control_report(
        phase2dq_report=phase2dq_report,
        case_dir=case_dir,
    )
    mutation: Mutation | None = control_spec["mutation"]
    if mutation is not None:
        mutation(control_report, case_dir)
    control_report_json = case_dir / "phase2dq_control_report.json"
    _write_json(control_report_json, control_report)
    validation = validate_phase2dq_replay_bundle_portability_summary(control_report)
    validation_report_json = case_dir / "phase2dq_validation.json"
...
def audit_phase2dr_portability_summary_negative_controls(
    *,
    phase2dq_report_json: str | Path,
    output_dir: str | Path,
    output_report_json: str | Path,
) -> dict[str, Any]:
    phase2dq_report = _read_json(phase2dq_report_json)
    ...
    negative_rows = [
        ...
        if row["control_id"] != "positive_control_original_phase2dq_summary"
    ]
    checks = {
        "source_phase2dq_passed": phase2dq_report.get("passed") is True,
        "positive_control_still_passes": any(
            row["control_id"] == "positive_control_original_phase2dq_summary"
    ...
    report = {
        "artifact_family": "phase2dr_portability_summary_negative_controls",
        "passed": passed,
        "ready_for_phase2dq_gate_strictness_claim": passed,
        ...
        "supported_claims": [
            (
                "the Phase2DQ portability-summary gate rejects source summary "
                "tampering, recorded check tampering, failed upstream replay "
                "validation, incomplete upstream negative controls, missing or "
                "malformed summary rows or columns, missing or weakened markdown "
                "evidence, metric mismatch, and overstated epoch claims"
            )
        ]
        if passed
        else [],
        ...
        "next_required_experiment": (
            ...
            if passed
            else "repair_phase2dr_portability_summary_negative_controls"
        ),
        "evidence": {
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Audit Phase2DR portability summary negative controls."
    )
    parser.add_argument("--phase2dq-report-json", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--output-report-json", required=True)
    parser.add_argument("--no-fail", action="store_true")
    args = parser.parse_args()
    report = audit_phase2dr_portability_summary_negative_controls(
        phase2dq_report_json=args.phase2dq_report_json,

...
TERMINAL_FILESYSTEM_FEATURES = 8
PREDICTION_FEEDBACK_FEATURES = 3
PREDICTION_FEEDBACK_START_INDEX = 31 + TERMINAL_FILESYSTEM_FEATURES
HARD_TASK_TYPES = {
...
_DIAGNOSTIC_FEEDBACK_INDICES = frozenset(
    range(
        PREDICTION_FEEDBACK_START_INDEX,
        PREDICTION_FEEDBACK_START_INDEX + PREDICTION_FEEDBACK_FEATURES,
    )
)
...
def _bounded_feedback_value(value: float | None, *, limit: float = 10.0) -> float:
    if value is None or not math.isfinite(float(value)):
        return 0.0
    clipped = max(min(float(value), limit), -limit)
    ...
    return "\n".join(
        [
            ...
            *(
            ...
            f"{state.runtime_evidence.model_prediction_error or 0.0:.6f}",
            "observed_prediction_error="
            f"{state.runtime_evidence.observed_prediction_error or 0.0:.6f}",
            "prediction_error_delta="
            f"{state.runtime_evidence.prediction_error_delta or 0.0:.6f}",
...
def _terminal_filesystem_numeric_features(state: SystemStateFrame) -> list[float]:
    file_change_visible = bool(
        state.filesystem.changed_paths
        or state.filesystem.dirty_files
    ...
    stdout_unread = bool(state.terminal.stdout_unread)
    stderr_unread = bool(state.terminal.stderr_unread)
    return [
        1.0 if stdout_unread else 0.0,
        1.0 if stderr_unread else 0.0,
        1.0 if state.terminal.last_output_channel == "stdout" else 0.0,
        1.0 if state.terminal.last_output_channel == "stderr" else 0.0,
        1.0 if state.filesystem.changed_paths else 0.0,
        1.0 if state.filesystem.dirty_files else 0.0,
        1.0 if stdout_unread and file_change_visible else 0.0,
        1.0 if stderr_unread and file_change_visible else 0.0,
    ]


def _prediction_feedback_numeric_features(state: SystemStateFrame) -> list[float]:
    return [
        _bounded_feedback_value(state.runtime_evidence.model_prediction_error),
        _bounded_feedback_value(state.runtime_evidence.observed_prediction_error),
        _bounded_feedback_value(state.runtime_evidence.prediction_error_delta),
    ]
...
def valid_action_mask(state: SystemStateFrame) -> np.ndarray:
    ...
    process_stale = bool(
        process_active
        and (
            state.process.resource_alert
            or state.process.last_output_ms >= 30_000
            or state.time.since_last_output_ms >= 30_000
        )
    )
    if not candidate_commands(state):
        mask[action_to_index(ActionType.RUN_COMMAND)] = 0.0
    if state.goal.task_type == TaskType.PROCESS_HANG and state.terminal.last_command:
        mask[action_to_index(ActionType.RUN_COMMAND)] = 0.0
    if process_active:
        mask[action_to_index(ActionType.RUN_COMMAND)] = 0.0
    ask_user_signal_visible = (
        bool(state.user.confirmation_required or state.user.user_block_requested)
        or bool(state.process.waiting_for_input or state.terminal.input_requested)
        and not state.user.manual_input_active
    )
    if not ask_user_signal_visible:
        mask[action_to_index(ActionType.ASK_USER)] = 0.0
    if state.user.manual_input_active:
        mask[action_to_index(ActionType.ASK_USER)] = 0.0
    pending_file_read = bool(state.filesystem.dirty_files or state.filesystem.changed_paths)
    config_file_read_signal = bool(
        state.goal.recovery_hint and "config" in state.goal.recovery_hint.lower()
    )
    if not candidate_files(state) or (not pending_file_read and not config_file_read_signal):
    ...
    stdout_visible = bool(state.terminal.stdout_delta or state.terminal.stdout_lines)
    stderr_visible = bool(state.terminal.stderr_delta or state.terminal.stderr_lines)
    unread_command_output_visible = bool(
        state.terminal.last_command
        and state.terminal.last_command != ActionType.READ_FILE.value
        and (
            (state.terminal.stdout_unread and stdout_visible)
            or (state.terminal.stderr_unread and stderr_visible)
        )
    )
    output_requires_terminal_read = (
        unread_command_output_visible
        or (
            not candidate_commands(state)
            and (
                (state.terminal.stdout_unread and stdout_visible)
                or (state.terminal.stderr_unread and stderr_visible)
            )
    ...
    if refresh_signal_visible and not unread_command_output_visible:
        mask[action_to_index(ActionType.READ_STDOUT)] = 0.0
        mask[action_to_index(ActionType.READ_STDERR)] = 0.0
    ready_to_run_command = bool(
        candidate_commands(state)
        and state.goal.task_type == TaskType.PROCESS_HANG
        and not process_active
        and not refresh_signal_visible
        and not pending_file_read
        and not unread_command_output_visible
        and not state.user.manual_input_active
        and not state.process.waiting_for_input
        and not state.terminal.input_requested
        and not state.terminal.last_command
    )
    if pending_file_read and not unread_command_output_visible:
        mask[action_to_index(ActionType.RUN_COMMAND)] = 0.0
    if pending_file_read and not unread_command_output_visible and not refresh_signal_visible:
        mask[action_to_index(ActionType.WAIT)] = 0.0
        mask[action_to_index(ActionType.DONE)] = 0.0
        mask[action_to_index(ActionType.REFRESH_STATE)] = 0.0
        mask[action_to_index(ActionType.ASK_USER)] = 0.0
    if output_requires_terminal_read:
        mask[action_to_index(ActionType.RUN_COMMAND)] = 0.0
        mask[action_to_index(ActionType.WAIT)] = 0.0
        mask[action_to_index(ActionType.DONE)] = 0.0
        mask[action_to_index(ActionType.REFRESH_STATE)] = 0.0
        mask[action_to_index(ActionType.READ_FILE)] = 0.0
        mask[action_to_index(ActionType.ASK_USER)] = 0.0
    if ready_to_run_command:
        mask[action_to_index(ActionType.WAIT)] = 0.0
        mask[action_to_index(ActionType.DONE)] = 0.0
    if process_active and not process_stale:
        mask[action_to_index(ActionType.STOP_PROCESS)] = 0.0
    if process_stale:
        mask[action_to_index(ActionType.RUN_COMMAND)] = 0.0
        mask[action_to_index(ActionType.WAIT)] = 0.0
        ...
        mask[action_to_index(ActionType.READ_STDERR)] = 0.0
        mask[action_to_index(ActionType.ASK_USER)] = 0.0
    if not stdout_visible or not state.terminal.stdout_unread:
        mask[action_to_index(ActionType.READ_STDOUT)] = 0.0
    if not stderr_visible or not state.terminal.stderr_unread:
        mask[action_to_index(ActionType.READ_STDERR)] = 0.0
    return mask
...
class StateVectorizer:
    ...
    def world_model_target_mask(self) -> np.ndarray:
        ...
        if self.numeric_features:
            ...
            for index in _STOCHASTIC_TELEMETRY_INDICES:
                mask[index] = 0.0
            for index in _DIAGNOSTIC_FEEDBACK_INDICES:
                mask[index] = 0.0
        return mask
    ...
    def vectorize_state(self, state: SystemStateFrame) -> np.ndarray:
        ...
        if self.numeric_features:
            ...
            numeric.extend(_terminal_filesystem_numeric_features(state))
            numeric.extend(_prediction_feedback_numeric_features(state))
...
def resolve_structured_action(
    ...
    if action_type == ActionType.RUN_COMMAND:
        ...
        if commands:
            ...
            command = _semantic_command_slot_override(command, commands, state)
        else:
...
def _semantic_command_slot_override(
    selected: str,
    commands: list[str],
    state: SystemStateFrame,
) -> str:
    """Prefer a command whose literal cues are explicitly requested by the goal.

    The learned command slot remains primary. This only corrects a slot when the
    goal text names a concrete token that appears in exactly one candidate
    command, e.g. selecting the command that prints a requested marker.
    """

    goal_text = " ".join(
        [
            state.goal.description,
            " ".join(state.goal.success_criteria),
            state.goal.recovery_hint or "",
        ]
    ).lower()
    if not goal_text:
        return selected
    negative_tokens = _negative_semantic_command_tokens(goal_text)
    goal_tokens = _semantic_command_tokens(goal_text) - negative_tokens
    if not goal_tokens:
        return selected
    selected_score = _semantic_command_score(selected, goal_tokens, negative_tokens)
    scored = [
        (_semantic_command_score(command, goal_tokens, negative_tokens), index, command)
        for index, command in enumerate(commands)
    ]
    scored.sort(key=lambda item: (-item[0], item[1]))
    best_score, _best_index, best_command = scored[0]
    if best_score > selected_score and best_score >= 1:
...
def _semantic_command_tokens(text: str) -> set[str]:
    return {
        token
...
def _semantic_command_score(
    command: str,
    positive_tokens: set[str],
    negative_tokens: set[str],
) -> int:
    command_tokens = _semantic_command_tokens(command)
    return len(command_tokens & positive_tokens) - len(command_tokens & negative_tokens)


def _negative_semantic_command_tokens(text: str) -> set[str]:
    tokens: set[str] = set()
    for match in re.finditer(r"\b(?:ignore|avoid|exclude)\b([^.;\n]*)", text.lower()):
        tokens.update(_semantic_command_tokens(match.group(1)))
    return tokens

...
REQUIRED_SOURCE_REPORT_ROLES: tuple[str, ...] = (
    "phase2em_report",
    "phase2en_negative_control_report",
    "phase2eo_replay_bundle_report",
    "phase2ep_negative_control_report",
)
...
REQUIRED_REPRODUCTION_STEPS: tuple[str, ...] = (
    ...
    "phase2eo_replay_bundle_summary_replay_bundle",
...
def _bundle_artifact_entries(phase2eo: dict[str, Any]) -> list[dict[str, Any]]:
    manifest = _read_json(phase2eo.get("evidence", {}).get("bundle_manifest"))
...
def _reproduction_steps(
    *,
    phase2em: dict[str, Any],
    phase2en: dict[str, Any],
    phase2eo: dict[str, Any],
    phase2em_report_json: str | Path,
    phase2en_report_json: str | Path,
    phase2eo_report_json: str | Path,
    phase2ep_report_json: str | Path,
) -> list[dict[str, Any]]:
    return [
        {
            "step_id": "phase2em_replay_bundle_summary_cross_directory_replay",
            "module": "reflexlm.cli.audit_phase2em_replay_bundle_summary_cross_directory_replay",
            "inputs": {"phase2el_report_json": phase2em.get("evidence", {}).get("phase2el_report_json")},
            "outputs": {
                "output_report_json": str(phase2em_report_json),
                "output_dir": phase2em.get("evidence", {}).get("replay_dir"),
            },
        },
        {
            "step_id": "phase2en_replay_bundle_summary_replay_negative_controls",
            "module": "reflexlm.cli.audit_phase2en_replay_bundle_summary_replay_negative_controls",
            "inputs": {"phase2em_report_json": str(phase2em_report_json)},
            "outputs": {
                "output_report_json": str(phase2en_report_json),
                "output_dir": phase2en.get("evidence", {}).get("negative_control_output_dir"),
            },
        },
        {
            "step_id": "phase2eo_replay_bundle_summary_replay_bundle",
            "module": "reflexlm.cli.audit_phase2eo_replay_bundle_summary_replay_bundle",
            "inputs": {"phase2en_report_json": str(phase2en_report_json)},
            "outputs": {
                "output_report_json": str(phase2eo_report_json),
                "output_dir": phase2eo.get("evidence", {}).get("bundle_dir"),
            },
        },
        {
            "step_id": "phase2ep_replay_bundle_summary_replay_bundle_negative_controls",
            "module": "reflexlm.cli.audit_phase2ep_replay_bundle_summary_replay_bundle_negative_controls",
            "inputs": {"phase2eo_report_json": str(phase2eo_report_json)},
            "outputs": {
                "output_report_json": str(phase2ep_report_json),
            },
        },
    ]


def validate_phase2eq_replay_bundle_summary_reproducibility_manifest(
    report: dict[str, Any],
) -> dict[str, Any]:
    ...
    checks = {
        "artifact_family_matches_phase2eq": (
            report.get("artifact_family")
            == "phase2eq_replay_bundle_summary_reproducibility_manifest"
        ),
        "top_level_phase2eq_passed": report.get("passed") is True,
        ...
        "source_phase2eo_and_phase2ep_passed": (
            report.get("source_summary", {}).get("phase2eo_passed") is True
            and report.get("source_summary", {}).get("phase2ep_passed") is True
        ),
        "source_phase2eo_validation_passed": (
            report.get("source_summary", {}).get("phase2eo_validation_passed") is True
        ),
        ...
        == report.get("source_summary", {}).get("phase2eo_copied_control_artifact_count"),
...
def audit_phase2eq_replay_bundle_summary_reproducibility_manifest(
    *,
    phase2ep_report_json: str | Path,
    output_manifest_json: str | Path,
    output_report_json: str | Path,
) -> dict[str, Any]:
    phase2ep = _read_json(phase2ep_report_json)
    phase2eo_report_json = phase2ep.get("evidence", {}).get("phase2eo_report_json")
    if not phase2eo_report_json:
        raise ValueError("Phase2EQ requires Phase2EP evidence.phase2eo_report_json")
    phase2eo = _read_json(phase2eo_report_json)
    phase2en_report_json = phase2eo.get("evidence", {}).get("phase2en_report_json")
    phase2em_report_json = phase2eo.get("evidence", {}).get("phase2em_report_json")
    if not phase2en_report_json or not phase2em_report_json:
        raise ValueError("Phase2EQ requires Phase2EO source report evidence paths")
    phase2en = _read_json(phase2en_report_json)
    phase2em = _read_json(phase2em_report_json)
    phase2eo_validation = validate_phase2eo_replay_bundle_summary_replay_bundle(
        phase2eo
    )
    source_reports = [
        _file_entry(role="phase2em_report", path=phase2em_report_json, content_type="application/json"),
        _file_entry(
            role="phase2en_negative_control_report",
            path=phase2en_report_json,
            content_type="application/json",
        ),
        _file_entry(
            role="phase2eo_replay_bundle_report",
            path=phase2eo_report_json,
            content_type="application/json",
        ),
        _file_entry(
            role="phase2ep_negative_control_report",
            path=phase2ep_report_json,
            content_type="application/json",
        ),
    ]
    bundle_artifacts = _bundle_artifact_entries(phase2eo)
    reproduction_steps = _reproduction_steps(
        phase2em=phase2em,
        phase2en=phase2en,
        phase2eo=phase2eo,
        phase2em_report_json=phase2em_report_json,
        phase2en_report_json=phase2en_report_json,
        phase2eo_report_json=phase2eo_report_json,
        phase2ep_report_json=phase2ep_report_json,
    )
    manifest = {
        "artifact_family": "phase2eq_replay_bundle_summary_reproducibility_manifest",
    ...
    checks = {
        "source_phase2ep_passed": phase2ep.get("passed") is True,
        "source_phase2eo_passed": phase2eo.get("passed") is True,
        "source_phase2eo_validation_passed": phase2eo_validation.get("passed") is True,
        "source_phase2ep_negative_controls_complete": (
            phase2ep.get("metrics", {}).get("negative_control_count")
            == phase2ep.get("metrics", {}).get("negative_controls_failed")
        ),
        ...
        "control_artifact_count_preserved": _control_artifact_count(bundle_artifacts)
        == phase2eo.get("metrics", {}).get("copied_control_artifact_count"),
    ...
    report = {
        ...
        "metrics": {
            ...
            "control_artifact_count": _control_artifact_count(bundle_artifacts),
            "reproduction_step_count": len(reproduction_steps),
            "phase2ep_negative_control_count": phase2ep.get("metrics", {}).get(
                "negative_control_count"
            ),
            "phase2ep_negative_controls_failed": phase2ep.get("metrics", {}).get(
                "negative_controls_failed"
            ),
        },
        "source_summary": {
            "phase2eo_passed": phase2eo.get("passed") is True,
            "phase2ep_passed": phase2ep.get("passed") is True,
            "phase2eo_validation_passed": phase2eo_validation.get("passed") is True,
            "phase2eo_copied_control_artifact_count": phase2eo.get("metrics", {}).get(
        ...
        "supported_claims": [
            (
                "reproducibility manifest for the bounded Phase2EO replay bundle "
                "and Phase2EP negative-control evidence"
            )
        ]
        if passed
        else [],
        ...
        "next_required_experiment": (
            ...
            if passed
            else "repair_phase2eq_replay_bundle_summary_reproducibility_manifest"
        ),
        "evidence": {
            "phase2ep_report_json": str(phase2ep_report_json),
            "phase2eo_report_json": str(phase2eo_report_json),
            "phase2en_report_json": str(phase2en_report_json),
            "phase2em_report_json": str(phase2em_report_json),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build Phase2EQ replay bundle summary reproducibility manifest."
    )
    parser.add_argument("--phase2ep-report-json", required=True)
    ...
    report = audit_phase2eq_replay_bundle_summary_reproducibility_manifest(
        phase2ep_report_json=args.phase2ep_report_json,

    ...
    *,
    ...
    for item in entries:
        ...
        if not source.exists():
            raise ValueError(f"Phase2DO source path missing: {source}")
...
def validate_phase2do_reproducibility_manifest_cross_directory_replay(
    report: dict[str, Any],
) -> dict[str, Any]:
    replay_report_path = report.get("evidence", {}).get("replayed_phase2dm_report")
    replay_validation = {}
    ...
    if replay_report_path:
        try:
            ...
            replay_validation = {}
    checks = {
        "artifact_family_matches_phase2do": (
            report.get("artifact_family")
            == "phase2do_reproducibility_manifest_cross_directory_replay"
        ),
        "top_level_phase2do_passed": report.get("passed") is True,
        ...
        "source_phase2dm_and_phase2dn_passed": (
            report.get("source_summary", {}).get("phase2dm_passed") is True
            and report.get("source_summary", {}).get("phase2dn_passed") is True
        ),
        "source_negative_controls_complete": (
            report.get("source_summary", {}).get("phase2dn_negative_control_count")
            == report.get("source_summary", {}).get("phase2dn_negative_controls_failed")
        ),
        "replayed_phase2dm_report_readable": replay_report_readable,
        "replayed_phase2dm_validation_passed": (
            replay_validation.get("passed") is True
        ),
...
def audit_phase2do_reproducibility_manifest_cross_directory_replay(
    *,
    phase2dn_report_json: str | Path,
    output_dir: str | Path,
    output_report_json: str | Path,
) -> dict[str, Any]:
    phase2dn = _read_json(phase2dn_report_json)
    phase2dm_report_json = phase2dn.get("evidence", {}).get("phase2dm_report_json")
    if not phase2dm_report_json:
        raise ValueError("Phase2DO requires Phase2DN evidence.phase2dm_report_json")
    phase2dm = _read_json(phase2dm_report_json)
    source_manifest_path = Path(
        str(phase2dm.get("evidence", {}).get("reproducibility_manifest", ""))
    )
    if not source_manifest_path.exists():
        raise ValueError("Phase2DO requires a readable Phase2DM manifest")
    ...
    replay_manifest_path = output_root / "phase2dm_replayed_manifest.json"
    _write_json(replay_manifest_path, replay_manifest)
    replay_phase2dm_report = deepcopy(phase2dm)
    replay_phase2dm_report.setdefault("evidence", {})[
        "reproducibility_manifest"
    ] = str(replay_manifest_path)
    replay_phase2dm_report_path = output_root / "phase2dm_replayed_report.json"
    _write_json(replay_phase2dm_report_path, replay_phase2dm_report)
    replay_validation = validate_phase2dm_publication_bundle_reproducibility_manifest(
        replay_phase2dm_report
    )
    replay_validation_path = output_root / "phase2dm_replayed_validation.json"
    ...
    checks = {
        "source_phase2dn_passed": phase2dn.get("passed") is True,
        "source_phase2dm_passed": phase2dm.get("passed") is True,
        "source_phase2dn_negative_controls_complete": (
            phase2dn.get("metrics", {}).get("negative_control_count")
            == phase2dn.get("metrics", {}).get("negative_controls_failed")
        ),
        "replay_manifest_written": replay_manifest_path.exists(),
        "replay_phase2dm_report_written": replay_phase2dm_report_path.exists(),
        "replay_phase2dm_validation_passed": replay_validation.get("passed") is True,
    ...
    passed = all(checks.values())
    report = {
        "artifact_family": "phase2do_reproducibility_manifest_cross_directory_replay",
        "passed": passed,
        ...
        "metrics": {
            ...
            "phase2dn_negative_control_count": phase2dn.get("metrics", {}).get(
                "negative_control_count"
            ),
            "phase2dn_negative_controls_failed": phase2dn.get("metrics", {}).get(
                "negative_controls_failed"
            ),
        },
        "source_summary": {
            "phase2dm_passed": phase2dm.get("passed") is True,
            "phase2dn_passed": phase2dn.get("passed") is True,
            "phase2dn_negative_control_count": phase2dn.get("metrics", {}).get(
                "negative_control_count"
            ),
            "phase2dn_negative_controls_failed": phase2dn.get("metrics", {}).get(
        ...
        "supported_claims": [
            (
                "cross-directory replay of the bounded Phase2DM reproducibility "
                "manifest with preserved source-report roles, bundle artifacts, "
                "hashes, reproduction steps, and boundary checks"
            )
        ]
        if passed
        else [],
        ...
        "next_required_experiment": (
            "phase2dp_cross_directory_replay_negative_controls"
            if passed
            else "repair_phase2do_reproducibility_manifest_cross_directory_replay"
        ),
        "evidence": {
            "phase2dn_report_json": str(phase2dn_report_json),
            ...
            "replayed_phase2dm_report": str(replay_phase2dm_report_path),
            "replayed_phase2dm_validation": str(replay_validation_path),
        },
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Replay Phase2DM reproducibility manifest in a separate directory."
    )
    parser.add_argument("--phase2dn-report-json", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--output-report-json", required=True)
    parser.add_argument("--no-fail", action="store_true")
    args = parser.parse_args()
    report = audit_phase2do_reproducibility_manifest_cross_directory_replay(
        phase2dn_report_json=args.phase2dn_report_json,

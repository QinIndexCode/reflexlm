...
from reflexlm.cli.audit_phase2ge_replay_bundle_summary_reproducibility_manifest import (
    ...
    audit_phase2ge_replay_bundle_summary_reproducibility_manifest,
...
def _phase2gd_fixture(tmp_path: Path) -> Path:
    _phase2gc_report(tmp_path)
    phase2gd = audit_phase2gd_replay_bundle_summary_replay_bundle_negative_controls(
        phase2gc_report_json=tmp_path / "phase2gc.json",
        output_dir=tmp_path / "gd_controls",
        output_report_json=tmp_path / "phase2gd.json",
    )
    assert phase2gd["passed"] is True
    return tmp_path / "phase2gd.json"


def _phase2ge_report(tmp_path: Path) -> dict:
    report = audit_phase2ge_replay_bundle_summary_reproducibility_manifest(
        phase2gd_report_json=_phase2gd_fixture(tmp_path),
        output_manifest_json=tmp_path / "phase2ge_manifest.json",
        output_report_json=tmp_path / "phase2ge.json",
    )
    assert report["passed"] is True
    return report


def test_phase2ge_accepts_and_rejects_reproducibility_manifest(tmp_path: Path) -> None:
    report = _phase2ge_report(tmp_path)
    validation = validate_phase2ge_replay_bundle_summary_reproducibility_manifest(report)
    manifest = json.loads(
    ...
    assert report["metrics"]["phase2gd_negative_control_count"] == report["metrics"][
        "phase2gd_negative_controls_failed"
    ]
    ...
    count_mismatch["source_summary"]["phase2gc_copied_control_artifact_count"] = 0
    validation = validate_phase2ge_replay_bundle_summary_reproducibility_manifest(
        count_mismatch
    )
    assert validation["passed"] is False
...
def _isolated_manifest_report(report: dict, tmp_path: Path, name: str) -> dict:
    ...
    phase2gc = json.loads(
        Path(report["evidence"]["phase2gc_report_json"]).read_text(encoding="utf-8")
    )
    bundle_root = Path(phase2gc["evidence"]["bundle_dir"])

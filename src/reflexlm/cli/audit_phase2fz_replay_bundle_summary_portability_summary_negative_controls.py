...
def _materialize_control_report(
    *,
    phase2fy_report: dict[str, Any],
    case_dir: Path,
) -> dict[str, Any]:
    control_report = deepcopy(phase2fy_report)
    source_markdown = _markdown_path(phase2fy_report)
    if not source_markdown.exists():
        raise ValueError("Phase2FZ requires a readable Phase2FY markdown summary")
    target_markdown = case_dir / "phase2fy_replay_bundle_summary_portability_summary.md"
...
def _mutate_source_summary_failed(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    report.setdefault("source_summary", {})["phase2fx_passed"] = False
...
def _mutate_phase2fw_validation_failed(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    report.setdefault("source_summary", {})["phase2fw_validation_passed"] = False


def _mutate_phase2fv_controls_incomplete(
...
def _mutate_phase2fx_controls_incomplete(
    report: dict[str, Any], case_dir: Path
) -> None:
    del case_dir
    report.setdefault("source_summary", {})["phase2fx_negative_controls_failed"] = 0
...
def _mutate_markdown_missing_dimension(report: dict[str, Any], case_dir: Path) -> None:
    ...
    if not isinstance(rows, list) or not rows:
        raise ValueError("Phase2FZ requires summary rows for markdown dimension control")
...
CONTROL_SPECS: tuple[dict[str, Any], ...] = (
    {
        "control_id": "positive_control_original_phase2fy_summary",
        "mutation": None,
    ...
    },
    {
        ...
        "expected_failed_checks": ["source_phase2fw_and_phase2fx_passed"],
    },
    {
    ...
    {
        "control_id": "negative_phase2fw_validation_failed",
        "mutation": _mutate_phase2fw_validation_failed,
        "expected_passed": False,
        "expected_failed_checks": ["source_phase2fw_validation_passed"],
    },
    {
        "control_id": "negative_phase2fv_controls_incomplete",
        "mutation": _mutate_phase2fv_controls_incomplete,
        "expected_passed": False,
        "expected_failed_checks": ["source_phase2fv_negative_controls_complete"],
    },
    {
        "control_id": "negative_phase2fx_controls_incomplete",
        "mutation": _mutate_phase2fx_controls_incomplete,
        "expected_passed": False,
        "expected_failed_checks": ["source_phase2fx_negative_controls_complete"],
    },
    {
...
def audit_phase2fz_replay_bundle_summary_portability_summary_negative_controls(
    *,
    phase2fy_report_json: str | Path,
    output_dir: str | Path,
    output_report_json: str | Path,
) -> dict[str, Any]:
    phase2fy_report = _read_json(phase2fy_report_json)
    ...
    negative_rows = [
        ...
        if row["control_id"] != "positive_control_original_phase2fy_summary"
    ]
    checks = {
        "source_phase2fy_passed": phase2fy_report.get("passed") is True,
        "positive_control_still_passes": any(
            row["control_id"] == "positive_control_original_phase2fy_summary"
    ...
    report = {
        ...
        "passed": passed,
        "ready_for_phase2fy_gate_strictness_claim": passed,
        ...
        "supported_claims": [
            (
                "the Phase2FY replay-bundle summary portability-summary gate "
                "rejects source summary tampering, recorded check tampering, "
                "failed FW validation, incomplete FV or FX negative controls, "
        ...
        "next_required_experiment": (
            ...
            if passed
            else "repair_phase2fz_replay_bundle_summary_portability_summary_negative_controls"
        ),
        "evidence": {
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Audit Phase2FZ replay bundle summary portability summary negative controls."
        )
    )
    parser.add_argument("--phase2fy-report-json", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--output-report-json", required=True)
    parser.add_argument("--no-fail", action="store_true")
    args = parser.parse_args()
    report = audit_phase2fz_replay_bundle_summary_portability_summary_negative_controls(
        phase2fy_report_json=args.phase2fy_report_json,

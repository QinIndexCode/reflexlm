...
def _phase2dw_fixture(tmp_path: Path) -> Path:
    ...
    return tmp_path / "phase2dw.json"


def test_phase2dx_rejects_replay_bundle_manifest_negative_controls(
    tmp_path: Path,
) -> None:
    phase2dw = _phase2dw_fixture(tmp_path)
    source_report = json.loads(phase2dw.read_text(encoding="utf-8"))

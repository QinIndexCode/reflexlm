...
def _phase2dt_fixture(tmp_path: Path) -> Path:
    ...
    return tmp_path / "phase2dt.json"


def _phase2du_report(tmp_path: Path) -> dict:
    report = audit_phase2du_portability_summary_replay_bundle(
        phase2dt_report_json=_phase2dt_fixture(tmp_path),
...
def test_phase2du_accepts_portability_summary_replay_bundle(tmp_path: Path) -> None:
    report = _phase2du_report(tmp_path)
    validation = validate_phase2du_portability_summary_replay_bundle(report)
    manifest = json.loads(
    ...
    assert report["metrics"]["phase2dt_negative_controls_failed"] == report[
        "metrics"
    ]["phase2dt_negative_control_count"]
...
def test_phase2du_validation_rejects_tampered_artifact(tmp_path: Path) -> None:
    report = _phase2du_report(tmp_path)
    manifest = json.loads(
        Path(report["evidence"]["bundle_manifest"]).read_text(encoding="utf-8")
    )
    markdown = next(
        ...
        if item["role"] == "replayed_portability_markdown"
    )
...
def test_phase2du_validation_rejects_missing_artifact_role(tmp_path: Path) -> None:
    report = _phase2du_report(tmp_path)
    ...
    manifest["artifacts"] = [
        item
        for item in manifest["artifacts"]
        if item["role"] != "replayed_phase2dq_validation"
    ]
...
def test_phase2du_validation_rejects_negative_controls_incomplete(
    tmp_path: Path,
) -> None:
    report = _phase2du_report(tmp_path)
    report["source_summary"]["phase2dt_negative_controls_failed"] = 0
    validation = validate_phase2du_portability_summary_replay_bundle(report)
...
def test_phase2du_validation_rejects_top_level_epoch_claim(tmp_path: Path) -> None:
    report = _phase2du_report(tmp_path)

...
from reflexlm.cli.audit_phase2ge_replay_bundle_summary_reproducibility_manifest import (
    CONTROL_ARTIFACT_ROLES,
    REQUIRED_ARTIFACT_ROLES,
    REQUIRED_REPRODUCTION_STEPS,
    REQUIRED_SOURCE_REPORT_ROLES,
    validate_phase2ge_replay_bundle_summary_reproducibility_manifest,
)
    ...
    *,
    ...
    for index, item in enumerate(entries):
        ...
        if not source.exists():
            raise ValueError(f"Phase2GG source path missing: {source}")
...
def validate_phase2gg_replay_bundle_summary_manifest_cross_directory_replay(
    report: dict[str, Any],
) -> dict[str, Any]:
    replay_report_path = report.get("evidence", {}).get("replayed_phase2ge_report")
    replay_validation = {}
    ...
    checks = {
        ...
        "source_phase2ge_and_phase2gf_passed": (
            report.get("source_summary", {}).get("phase2ge_passed") is True
            and report.get("source_summary", {}).get("phase2gf_passed") is True
        ),
        "source_negative_controls_complete": (
            report.get("source_summary", {}).get("phase2gf_negative_control_count")
            == report.get("source_summary", {}).get("phase2gf_negative_controls_failed")
        ),
        "replayed_phase2ge_report_readable": replay_report_readable,
        "replayed_phase2ge_validation_passed": replay_validation.get("passed") is True,
        ...
        "replayed_control_artifact_count_preserved": (
            ...
            == report.get("source_summary", {}).get("phase2ge_control_artifact_count")
        ),
...
def audit_phase2gg_replay_bundle_summary_manifest_cross_directory_replay(
    *,
    phase2gf_report_json: str | Path,
    output_dir: str | Path,
    output_report_json: str | Path,
) -> dict[str, Any]:
    phase2gf = _read_json(phase2gf_report_json)
    phase2ge_report_json = phase2gf.get("evidence", {}).get("phase2ge_report_json")
    if not phase2ge_report_json:
        raise ValueError("Phase2GG requires Phase2GF evidence.phase2ge_report_json")
    phase2ge = _read_json(phase2ge_report_json)
    source_manifest_path = Path(
        str(phase2ge.get("evidence", {}).get("reproducibility_manifest", ""))
    )
    if not source_manifest_path.exists():
        raise ValueError("Phase2GG requires a readable Phase2GE manifest")
    ...
    replay_manifest_path = output_root / "phase2ge_replayed_manifest.json"
    _write_json(replay_manifest_path, replay_manifest)
    replay_phase2ge_report = deepcopy(phase2ge)
    replay_phase2ge_report.setdefault("evidence", {})[
        "reproducibility_manifest"
    ] = str(replay_manifest_path)
    replay_phase2ge_report_path = output_root / "phase2ge_replayed_report.json"
    _write_json(replay_phase2ge_report_path, replay_phase2ge_report)
    replay_validation = (
        validate_phase2ge_replay_bundle_summary_reproducibility_manifest(
            replay_phase2ge_report
        )
    )
    replay_validation_path = output_root / "phase2ge_replayed_validation.json"
    ...
    checks = {
        "source_phase2gf_passed": phase2gf.get("passed") is True,
        "source_phase2ge_passed": phase2ge.get("passed") is True,
        "source_phase2gf_negative_controls_complete": (
            phase2gf.get("metrics", {}).get("negative_control_count")
            == phase2gf.get("metrics", {}).get("negative_controls_failed")
        ),
        "replay_manifest_written": replay_manifest_path.exists(),
        "replay_phase2ge_report_written": replay_phase2ge_report_path.exists(),
        "replay_phase2ge_validation_passed": replay_validation.get("passed") is True,
        ...
        == phase2ge.get("metrics", {}).get("control_artifact_count"),
    ...
    passed = all(checks.values())
    report = {
        "artifact_family": "phase2gg_replay_bundle_summary_manifest_cross_directory_replay",
        "passed": passed,
        ...
        "metrics": {
            ...
            "phase2gf_negative_control_count": phase2gf.get("metrics", {}).get(
                "negative_control_count"
            ),
            "phase2gf_negative_controls_failed": phase2gf.get("metrics", {}).get(
                "negative_controls_failed"
            ),
        },
        "source_summary": {
            "phase2ge_passed": phase2ge.get("passed") is True,
            "phase2gf_passed": phase2gf.get("passed") is True,
            "phase2ge_control_artifact_count": phase2ge.get("metrics", {}).get(
        ...
        "supported_claims": [
            (
                "cross-directory replay of the bounded Phase2GE reproducibility "
        ...
        "next_required_experiment": (
            ...
            if passed
            else "repair_phase2gg_replay_bundle_summary_manifest_cross_directory_replay"
        ),
        "evidence": {
            "phase2gf_report_json": str(phase2gf_report_json),
            "phase2ge_report_json": str(phase2ge_report_json),
            ...
            "replayed_phase2ge_report": str(replay_phase2ge_report_path),
            "replayed_phase2ge_validation": str(replay_validation_path),
        },
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Replay Phase2GE reproducibility manifest in a separate directory."
    )
    parser.add_argument("--phase2gf-report-json", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--output-report-json", required=True)
    parser.add_argument("--no-fail", action="store_true")
    args = parser.parse_args()
    report = audit_phase2gg_replay_bundle_summary_manifest_cross_directory_replay(
        phase2gf_report_json=args.phase2gf_report_json,

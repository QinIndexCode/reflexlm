...
def _command_identity_diagnostics(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for row in rows:
        if int(row.get("command_slot", -100)) == -100:
            continue
        command_rows += 1
        ref = _dict(row.get("nsi_reference"))
...
def phase2u_repair_trace_to_head_row(row: dict[str, Any]) -> dict[str, Any]:
    if row.get("phase") != "Phase2U":
        raise ValueError(f"expected Phase2U trace row: {row.get('trace_id')}")
    if row.get("trace_construction_mode") != TRACE_CONSTRUCTION_MODE:
        raise ValueError(f"expected Phase2U baseline-feasible trace: {row.get('trace_id')}")
    head_row = phase2s_repair_trace_to_head_row(row)
    original_prompt = str(head_row.get("state_prompt") or "")
    head_row["prompt_style"] = "phase2u_baseline_feasible_repair_head_v1"
    head_row["state_prompt"] = original_prompt.replace(
        "Phase2S public repository repair native-head state input.",
        "Phase2U baseline-feasible public repair-control native-head state input.",
        1,
    ) + "\n\nPhase2U repair-control constraints:\n- Select a bounded repair-loop command using only runtime-visible public repository evidence.\n- Baseline feasibility controls are measured separately; do not infer from oracle/reference fields.\n- This row is non-sealed and cannot be used to tune sealed transfer results.\n"
    head_row["runtime_overrides"] = [
        "debug_cortex_escalation",
        "phase2u_baseline_feasible_repair_controls",
        "sandboxed_patch_test_rollback_stop_evidence",
    ]
    source_trace = _dict(head_row.get("source_trace"))
    source_trace.update(
        {
            "phase": "Phase2U",
            "benchmark_family": row.get("benchmark_family"),
            "trace_construction_mode": row.get("trace_construction_mode"),
            "phase2u_subset": row.get("phase2u_subset"),
            "phase2u_source_trace_id": row.get("phase2u_source_trace_id"),
            "sealed_v3_used": False,
        }
    )
    head_row["source_trace"] = source_trace
    return head_row


def build_phase2u_head_dataset(
    *,
    ...
    train_rows = [phase2u_repair_trace_to_head_row(row) for row in train_raw_rows]
    val_rows = [phase2u_repair_trace_to_head_row(row) for row in val_raw_rows]
    holdout_rows = [phase2u_repair_trace_to_head_row(row) for row in holdout_raw_rows]
    ...
    effective_split_hashes = (
        ...
        if data_health
            ...
            "phase2u_train": _sha256(train_raw_rows),
            "phase2u_val": _sha256(val_raw_rows),
            **({"phase2u_holdout": _sha256(holdout_raw_rows)} if holdout_jsonl else {}),
        }
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build native-head training rows from Phase2U baseline-feasible repair-control traces."
    )

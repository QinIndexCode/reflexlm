...
def _copy_control_evidence(
    *,
    phase2ev: dict[str, Any],
    target_dir: Path,
) -> list[dict[str, Any]]:
    copied_rows = []
    target_dir.mkdir(parents=True, exist_ok=True)
    for index, row in enumerate(_control_rows(phase2ev)):
...
def validate_phase2ew_replay_bundle_summary_cross_directory_replay(
    report: dict[str, Any],
) -> dict[str, Any]:
    replay_report_path = report.get("evidence", {}).get("replayed_phase2eu_report")
    replay_validation = {}
    ...
    if replay_report_path:
        try:
            ...
            replay_validation = validate_phase2eu_replay_bundle_summary_portability_summary(
                replay_report
            )
    ...
    checks = {
        "artifact_family_matches_phase2ew": (
            report.get("artifact_family")
            == "phase2ew_replay_bundle_summary_cross_directory_replay"
        ),
        "top_level_phase2ew_passed": report.get("passed") is True,
        ...
        "source_phase2eu_and_phase2ev_passed": (
            report.get("source_summary", {}).get("phase2eu_passed") is True
            and report.get("source_summary", {}).get("phase2ev_passed") is True
        ),
        "source_phase2eu_validation_passed": (
            report.get("source_summary", {}).get("phase2eu_validation_passed") is True
        ),
        "source_negative_controls_complete": (
            report.get("source_summary", {}).get("phase2ev_negative_control_count")
            == report.get("source_summary", {}).get("phase2ev_negative_controls_failed")
        ),
        "replayed_phase2eu_report_readable": replay_report_readable,
        "replayed_phase2eu_validation_passed": replay_validation.get("passed") is True,
...
def audit_phase2ew_replay_bundle_summary_cross_directory_replay(
    *,
    phase2ev_report_json: str | Path,
    output_dir: str | Path,
    output_report_json: str | Path,
) -> dict[str, Any]:
    phase2ev = _read_json(phase2ev_report_json)
    phase2eu_report_json = phase2ev.get("evidence", {}).get("phase2eu_report_json")
    if not phase2eu_report_json:
        raise ValueError("Phase2EW requires Phase2EV evidence.phase2eu_report_json")
    phase2eu = _read_json(phase2eu_report_json)
    source_validation = validate_phase2eu_replay_bundle_summary_portability_summary(
        phase2eu
    )
    source_markdown_path = Path(
        str(phase2eu.get("evidence", {}).get("portability_summary_markdown", ""))
    )
    if not source_markdown_path.exists():
        raise ValueError("Phase2EW requires a readable Phase2EU markdown summary")
    output_root = Path(output_dir)
    output_root.mkdir(parents=True, exist_ok=True)
    replay_markdown_path = output_root / "phase2eu_replayed_portability_summary.md"
    shutil.copy2(source_markdown_path, replay_markdown_path)
    replay_phase2eu_report = deepcopy(phase2eu)
    replay_phase2eu_report.setdefault("evidence", {})[
        "portability_summary_markdown"
    ] = str(replay_markdown_path)
    replay_phase2eu_report_path = output_root / "phase2eu_replayed_report.json"
    _write_json(replay_phase2eu_report_path, replay_phase2eu_report)
    replay_validation = validate_phase2eu_replay_bundle_summary_portability_summary(
        replay_phase2eu_report
    )
    replay_validation_path = output_root / "phase2eu_replayed_validation.json"
    ...
    replayed_control_rows_path = output_root / "phase2ev_replayed_control_results.json"
    _write_json(replayed_control_rows_path, {"control_results": replayed_control_rows})
    source_rows = phase2eu.get("summary_table", {}).get("rows", [])
    replay_rows = replay_phase2eu_report.get("summary_table", {}).get("rows", [])
    source_columns = phase2eu.get("summary_table", {}).get("columns", [])
    replay_columns = replay_phase2eu_report.get("summary_table", {}).get("columns", [])
    ...
    replayed_negative_controls_failed = sum(
        ...
        if row.get("control_id") != "positive_control_original_phase2eu_summary"
    )
    ...
    checks = {
        "source_phase2ev_passed": phase2ev.get("passed") is True,
        "source_phase2eu_passed": phase2eu.get("passed") is True,
        "source_phase2eu_validation_passed": source_validation.get("passed") is True,
        "source_phase2ev_negative_controls_complete": (
            phase2ev.get("metrics", {}).get("negative_control_count")
            == phase2ev.get("metrics", {}).get("negative_controls_failed")
        ),
        "replay_phase2eu_report_written": replay_phase2eu_report_path.exists(),
        "replay_phase2eu_validation_passed": replay_validation.get("passed") is True,
        ...
        == phase2ev.get("metrics", {}).get("control_count"),
        ...
        == phase2ev.get("metrics", {}).get("expected_failed_check_assertions"),
        "replay_directory_is_distinct": source_markdown_path.parent.resolve()
        != replay_markdown_path.parent.resolve(),
    }
    passed = all(checks.values())
    report = {
        "artifact_family": "phase2ew_replay_bundle_summary_cross_directory_replay",
        "passed": passed,
        ...
        "metrics": {
            ...
            "phase2ev_negative_control_count": phase2ev.get("metrics", {}).get(
                "negative_control_count"
            ),
            "phase2ev_negative_controls_failed": phase2ev.get("metrics", {}).get(
                "negative_controls_failed"
            ),
        },
        "source_summary": {
            "phase2eu_passed": phase2eu.get("passed") is True,
            "phase2ev_passed": phase2ev.get("passed") is True,
            "phase2eu_validation_passed": source_validation.get("passed") is True,
            "phase2ev_negative_control_count": phase2ev.get("metrics", {}).get(
                "negative_control_count"
            ),
            "phase2ev_negative_controls_failed": phase2ev.get("metrics", {}).get(
        ...
        "replay_summary": {
            ...
            "source_control_count": phase2ev.get("metrics", {}).get("control_count"),
            "replayed_control_count": len(replayed_control_rows),
            "source_negative_controls_failed": phase2ev.get("metrics", {}).get(
                "negative_controls_failed"
            ),
            "replayed_negative_controls_failed": replayed_negative_controls_failed,
            "source_expected_failed_check_assertions": phase2ev.get("metrics", {}).get(
        ...
        "supported_claims": [
            (
                "cross-directory replay of the bounded Phase2EU replay-bundle "
                "summary portability summary with preserved markdown hash, summary "
                "rows, summary columns, validator pass, copied Phase2EV control "
        ...
        "next_required_experiment": (
            ...
            if passed
            else "repair_phase2ew_replay_bundle_summary_cross_directory_replay"
        ),
        "evidence": {
            "phase2ev_report_json": str(phase2ev_report_json),
            "phase2eu_report_json": str(phase2eu_report_json),
            "source_markdown": str(source_markdown_path),
            "replay_dir": str(output_root),
            "replayed_markdown": str(replay_markdown_path),
            "replayed_phase2eu_report": str(replay_phase2eu_report_path),
            "replayed_phase2eu_validation": str(replay_validation_path),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Replay Phase2EU replay-bundle summary in a separate directory."
    )
    parser.add_argument("--phase2ev-report-json", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--output-report-json", required=True)
    parser.add_argument("--no-fail", action="store_true")
    args = parser.parse_args()
    report = audit_phase2ew_replay_bundle_summary_cross_directory_replay(
        phase2ev_report_json=args.phase2ev_report_json,

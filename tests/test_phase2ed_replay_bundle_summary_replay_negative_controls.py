...
def _phase2ec_fixture(tmp_path: Path) -> Path:
    _phase2ec_report(tmp_path)
    return tmp_path / "phase2ec.json"


def test_phase2ed_rejects_replay_bundle_summary_replay_negative_controls(
    tmp_path: Path,
) -> None:
    phase2ec = _phase2ec_fixture(tmp_path)
    source_replay_dir = tmp_path / "ec_replay"

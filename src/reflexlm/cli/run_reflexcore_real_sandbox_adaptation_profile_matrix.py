...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Run ReflexCore V0 mixed real-sandbox adaptation once per seed "
            "and reuse each checkpoint across evaluation profiles."
        ),
    )
    ...
    parser.add_argument("--eval-profile", action="append", default=[])
    ...
    parser.add_argument("--real-sandbox-live-observation", action="store_true")
    ...
    parser.add_argument("--min-offline-margin", type=float, default=0.0)
    parser.add_argument("--min-closed-loop-margin", type=float, default=0.0)
    parser.add_argument("--min-real-sandbox-margin", type=float, default=0.0)
    parser.add_argument("--min-real-sandbox-success-rate", type=float, default=0.0)
    args = parser.parse_args()

    seeds = tuple(args.seed) if args.seed else (13, 17, 23)
    eval_profiles = tuple(args.eval_profile) if args.eval_profile else (
        "default",
        "hard",
        "wide_ood",
    )
    report = run_reflexcore_real_sandbox_adaptation_profile_matrix(
        ReflexCoreRealSandboxAdaptationProfileMatrixConfig(
            ...
            real_sandbox_live_observation=args.real_sandbox_live_observation,
            ...
            min_offline_margin=args.min_offline_margin,

...
def validate_phase2homeostasis_reproducibility_manifest_replay(
    report: dict[str, Any],
) -> dict[str, Any]:
    ...
    checks = {
        "artifact_family_matches": (
            ...
            == "phase2homeostasis_reproducibility_manifest_replay"
        ),
        "top_level_passed": report.get("passed") is True,
        "top_level_ready_claim_is_bounded": (
            report.get("ready_for_bounded_homeostasis_manifest_replay_claim") is True
        ...
        "source_reproducibility_manifest_passed": (
...
def audit_phase2homeostasis_reproducibility_manifest_replay(
    *,
    ...
    if not source_manifest_path.exists():
        raise ValueError("replay requires a readable source reproducibility manifest")
    ...
    replay_manifest_path = output_root / "replayed_reproducibility_manifest.json"
    ...
    replay_report_path = output_root / "replayed_reproducibility_report.json"
    _write_json(replay_report_path, replay_report)
    replay_validation = validate_phase2homeostasis_publication_reproducibility_manifest(
        replay_report
    )
    replay_validation_path = output_root / "replayed_reproducibility_validation.json"
    ...
    report = {
        ...
        "passed": passed,
        "ready_for_bounded_homeostasis_manifest_replay_claim": passed,
        ...
        "claim_boundary": (
            "This replay verifies that the bounded HMAC-authenticated homeostatic "
            "publication manifest can be copied to a distinct directory and "
            "revalidated by hash. It is not a fresh model rerun and does not support "
            "unbounded memory, open-ended native perception, production autonomy, "
            "or epoch-making architecture claims."
        ),
        "supported_claims": [
            "bounded cross-directory replay of the homeostatic reproducibility manifest"
        ]
        if passed
        else [],
        "unsupported_claims": [
        ...
        "next_required_experiment": (
            "fresh_model_rerun_from_reproducibility_manifest"
            if passed
            else "repair_homeostatic_manifest_replay"
        ),
        "evidence": {
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Replay a Phase2Homeostasis reproducibility manifest in a fresh directory."
    )
    ...
    report = audit_phase2homeostasis_reproducibility_manifest_replay(

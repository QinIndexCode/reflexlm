...
def _phase2dy_fixture(tmp_path: Path) -> Path:
    ...
    phase2dy = audit_phase2dy_replay_bundle_cross_directory_replay(
        phase2dx_report_json=tmp_path / "phase2dx.json",
        output_dir=tmp_path / "dy_replay",
        output_report_json=tmp_path / "phase2dy.json",
    )
    assert phase2dy["passed"] is True
    return tmp_path / "phase2dy.json"


def test_phase2dz_rejects_replay_bundle_cross_directory_negative_controls(
    tmp_path: Path,
) -> None:
    phase2dy = _phase2dy_fixture(tmp_path)
    source_report = json.loads(phase2dy.read_text(encoding="utf-8"))
    source_replay_dir = Path(source_report["evidence"]["replay_dir"])
    source_replay_files = sorted(path.name for path in source_replay_dir.rglob("*") if path.is_file())
    ...
    assert report["metrics"]["negative_control_count"] >= 14
    ...
    assert sorted(path.name for path in source_replay_dir.rglob("*") if path.is_file()) == source_replay_files
    assert report["ready_for_epoch_making_architecture_claim"] is False
    assert report["next_required_experiment"] == "phase2ea_replay_bundle_portability_summary"

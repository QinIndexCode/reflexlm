...
from reflexlm.cli.audit_phase2fw_replay_bundle_summary_manifest_cross_directory_replay import (
    audit_phase2fw_replay_bundle_summary_manifest_cross_directory_replay,
...
def _phase2fv_fixture(tmp_path: Path) -> Path:
    _phase2fu_report(tmp_path)
    phase2fv = audit_phase2fv_replay_bundle_summary_manifest_negative_controls(
        phase2fu_report_json=tmp_path / "phase2fu.json",
        output_dir=tmp_path / "fv_controls",
        output_report_json=tmp_path / "phase2fv.json",
    )
    assert phase2fv["passed"] is True
    return tmp_path / "phase2fv.json"


def _phase2fw_report(tmp_path: Path) -> dict:
    report = audit_phase2fw_replay_bundle_summary_manifest_cross_directory_replay(
        phase2fv_report_json=_phase2fv_fixture(tmp_path),
        output_dir=tmp_path / "fw_replay",
        output_report_json=tmp_path / "phase2fw.json",
    )
    assert report["passed"] is True
    return report


def test_phase2fw_accepts_manifest_cross_directory_replay(tmp_path: Path) -> None:
    report = _phase2fw_report(tmp_path)
    ...
    assert report["metrics"]["replayed_control_artifact_count"] == report[
        "source_summary"
    ]["phase2fu_control_artifact_count"]
    assert report["metrics"]["phase2fv_negative_control_count"] == report["metrics"][
...
def test_phase2fw_validation_rejects_negative_matrix(tmp_path: Path) -> None:
    base_report = _phase2fw_report(tmp_path)
    ...
    assert validation["checks"]["replayed_phase2fu_validation_passed"] is False

    report = _isolated_replay_report(base_report, tmp_path, "missing_report")
    Path(report["evidence"]["replayed_phase2fu_report"]).unlink()
    validation = validate_phase2fw_replay_bundle_summary_manifest_cross_directory_replay(
        report
    )
    assert validation["passed"] is False
    assert validation["checks"]["replayed_phase2fu_report_readable"] is False
...
def _isolated_replay_report(report: dict, tmp_path: Path, name: str) -> dict:
    ...
    replayed_report = Path(isolated["evidence"]["replayed_phase2fu_report"])

...
from reflexlm.cli.audit_phase2dd_cross_order_negative_controls import (
    audit_phase2dd_cross_order_negative_controls,
)
...
def _phase2dc_fixture(tmp_path: Path) -> Path:
    ...
    return tmp_path / "phase2dc.json"


def test_phase2dd_rejects_cross_order_negative_controls(tmp_path: Path) -> None:
    report = audit_phase2dd_cross_order_negative_controls(
        phase2dc_report_json=_phase2dc_fixture(tmp_path),
        output_dir=tmp_path / "controls",
        output_report_json=tmp_path / "phase2dd.json",
    )

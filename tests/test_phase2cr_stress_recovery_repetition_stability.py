...
from reflexlm.cli.audit_phase2cr_stress_recovery_repetition_stability import (
    audit_phase2cr_stress_recovery_repetition_stability,
)
...
def _phase2cp_fixture(tmp_path: Path) -> Path:
    ...
    return _write(
        tmp_path / "phase2cp.json",
        {
            "passed": True,
            "metrics": {
                "runtime_paths": [runtime for runtime, _, _ in runtime_specs],
                "python_versions": [version for _, version, _ in runtime_specs],
            },
            "evidence": {
...
def test_phase2cr_accepts_repeated_stable_phase2cp_audits(tmp_path: Path) -> None:
    report = audit_phase2cr_stress_recovery_repetition_stability(
        phase2cp_report_json=_phase2cp_fixture(tmp_path),
        output_dir=tmp_path / "repetitions",
        output_report_json=tmp_path / "phase2cr.json",
        repetition_count=3,
    )

    assert report["passed"] is True
    assert report["checks"]["all_repetition_signatures_match_reference"] is True
    assert report["metrics"]["repetition_count"] == 3
    assert report["metrics"]["signature_mismatch_count"] == 0
    assert report["ready_for_epoch_making_architecture_claim"] is False


def test_phase2cr_rejects_repetition_metric_drift(tmp_path: Path) -> None:
    report = audit_phase2cr_stress_recovery_repetition_stability(
        phase2cp_report_json=_phase2cp_fixture(tmp_path),
        output_dir=tmp_path / "repetitions",
        output_report_json=tmp_path / "phase2cr.json",
        repetition_count=3,
        inject_drift_repetition_index=1,
    )

    assert report["passed"] is False
    assert report["checks"]["all_repetition_audits_passed"] is False
    assert report["checks"]["all_repetition_signatures_match_reference"] is False
    assert report["metrics"]["signature_mismatch_count"] == 1

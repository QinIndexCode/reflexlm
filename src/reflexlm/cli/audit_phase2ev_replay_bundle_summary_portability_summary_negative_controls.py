...
def _materialize_control_report(
    *,
    phase2eu_report: dict[str, Any],
    case_dir: Path,
) -> dict[str, Any]:
    control_report = deepcopy(phase2eu_report)
    source_markdown = _markdown_path(phase2eu_report)
    if not source_markdown.exists():
        raise ValueError("Phase2EV requires a readable Phase2EU markdown summary")
    target_markdown = case_dir / "phase2eu_replay_bundle_summary_portability_summary.md"
...
def _mutate_source_summary_failed(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    report.setdefault("source_summary", {})["phase2et_passed"] = False
...
def _mutate_phase2es_validation_failed(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    report.setdefault("source_summary", {})["phase2es_validation_passed"] = False


def _mutate_phase2er_controls_incomplete(
...
def _mutate_phase2et_controls_incomplete(
    report: dict[str, Any], case_dir: Path
) -> None:
    del case_dir
    report.setdefault("source_summary", {})["phase2et_negative_controls_failed"] = 0
...
def _mutate_markdown_missing_dimension(report: dict[str, Any], case_dir: Path) -> None:
    ...
    if not isinstance(rows, list) or not rows:
        raise ValueError("Phase2EV requires summary rows for markdown dimension control")
...
CONTROL_SPECS: tuple[dict[str, Any], ...] = (
    {
        "control_id": "positive_control_original_phase2eu_summary",
        "mutation": None,
    ...
    },
    {
        ...
        "expected_failed_checks": ["source_phase2es_and_phase2et_passed"],
    },
    {
    ...
    {
        "control_id": "negative_phase2es_validation_failed",
        "mutation": _mutate_phase2es_validation_failed,
        "expected_passed": False,
        "expected_failed_checks": ["source_phase2es_validation_passed"],
    },
    {
        "control_id": "negative_phase2er_controls_incomplete",
        "mutation": _mutate_phase2er_controls_incomplete,
        "expected_passed": False,
        "expected_failed_checks": ["source_phase2er_negative_controls_complete"],
    },
    {
        "control_id": "negative_phase2et_controls_incomplete",
        "mutation": _mutate_phase2et_controls_incomplete,
        "expected_passed": False,
        "expected_failed_checks": ["source_phase2et_negative_controls_complete"],
    },
    {
...
def _run_control(
    *,
    ...
    control_report = _materialize_control_report(
        phase2eu_report=phase2eu_report,
        case_dir=case_dir,
    )
    mutation: Mutation | None = control_spec["mutation"]
    if mutation is not None:
        mutation(control_report, case_dir)
    control_report_json = case_dir / "phase2eu_control_report.json"
    _write_json(control_report_json, control_report)
    validation = validate_phase2eu_replay_bundle_summary_portability_summary(
        control_report
    )
    validation_report_json = case_dir / "phase2eu_validation.json"
...
def audit_phase2ev_replay_bundle_summary_portability_summary_negative_controls(
    *,
    phase2eu_report_json: str | Path,
    output_dir: str | Path,
    output_report_json: str | Path,
) -> dict[str, Any]:
    phase2eu_report = _read_json(phase2eu_report_json)
    ...
    negative_rows = [
        ...
        if row["control_id"] != "positive_control_original_phase2eu_summary"
    ]
    checks = {
        "source_phase2eu_passed": phase2eu_report.get("passed") is True,
        "positive_control_still_passes": any(
            row["control_id"] == "positive_control_original_phase2eu_summary"
    ...
    report = {
        ...
        "passed": passed,
        "ready_for_phase2eu_gate_strictness_claim": passed,
        ...
        "supported_claims": [
            (
                "the Phase2EU replay-bundle summary portability-summary gate "
                "rejects source summary tampering, recorded check tampering, "
                "failed ES validation, incomplete ER or ET negative controls, "
        ...
        "next_required_experiment": (
            ...
            if passed
            else "repair_phase2ev_replay_bundle_summary_portability_summary_negative_controls"
        ),
        "evidence": {
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Audit Phase2EV replay bundle summary portability summary negative controls."
        )
    )
    parser.add_argument("--phase2eu-report-json", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--output-report-json", required=True)
    parser.add_argument("--no-fail", action="store_true")
    args = parser.parse_args()
    report = audit_phase2ev_replay_bundle_summary_portability_summary_negative_controls(
        phase2eu_report_json=args.phase2eu_report_json,

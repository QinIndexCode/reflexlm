...
def _copy_replay_tree(
    *,
    phase2ew_report: dict[str, Any],
    case_dir: Path,
) -> dict[str, str]:
    source_replay_dir = Path(str(phase2ew_report.get("evidence", {}).get("replay_dir", "")))
    if not source_replay_dir.exists():
        raise ValueError("Phase2EX requires a readable Phase2EW replay directory")
...
def _materialize_control_report(
    *,
    ...
    control_report = deepcopy(phase2ew_report)
    path_map = _copy_replay_tree(phase2ew_report=phase2ew_report, case_dir=case_dir)
    ...
    replayed_report_path = Path(
        str(control_report.get("evidence", {}).get("replayed_phase2eu_report", ""))
    )
...
def _replayed_report_path(report: dict[str, Any]) -> Path:
    return Path(str(report.get("evidence", {}).get("replayed_phase2eu_report", "")))
...
def _first_replayed_control_path(report: dict[str, Any], key: str) -> Path:
    ...
    if not isinstance(rows, list) or not rows or not isinstance(rows[0], dict):
        raise ValueError("Phase2EX requires replayed control results")
    return Path(str(rows[0].get(key, "")))


def _mutate_source_summary_failed(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    report.setdefault("source_summary", {})["phase2eu_passed"] = False
...
def _mutate_source_phase2eu_validation_failed(
    report: dict[str, Any], case_dir: Path
) -> None:
    del case_dir
    report.setdefault("source_summary", {})["phase2eu_validation_passed"] = False
...
def _mutate_source_negative_controls_incomplete(
    ...
    del case_dir
    report.setdefault("source_summary", {})["phase2ev_negative_controls_failed"] = 0
...
CONTROL_SPECS: tuple[dict[str, Any], ...] = (
    {
        "control_id": "positive_control_original_phase2ew_replay",
        "mutation": None,
    ...
    },
    {
        ...
        "expected_failed_checks": ["source_phase2eu_and_phase2ev_passed"],
    },
    {
    ...
    {
        "control_id": "negative_source_phase2eu_validation_failed",
        "mutation": _mutate_source_phase2eu_validation_failed,
        "expected_passed": False,
        "expected_failed_checks": ["source_phase2eu_validation_passed"],
    },
    {
    ...
    },
    {
        ...
        "expected_passed": False,
        "expected_failed_checks": [
            "replayed_phase2eu_report_readable",
            "replayed_phase2eu_validation_passed",
        ],
    },
    {
...
def _run_control(
    *,
    ...
    control_report = _materialize_control_report(
        phase2ew_report=phase2ew_report,
        case_dir=case_dir,
    )
    mutation: Mutation | None = control_spec["mutation"]
    if mutation is not None:
        mutation(control_report, case_dir)
    control_report_json = case_dir / "phase2ew_control_report.json"
    _write_json(control_report_json, control_report)
    validation = validate_phase2ew_replay_bundle_summary_cross_directory_replay(
        control_report
    )
    validation_report_json = case_dir / "phase2ew_validation.json"
...
def audit_phase2ex_replay_bundle_summary_replay_negative_controls(
    *,
    phase2ew_report_json: str | Path,
    output_dir: str | Path,
    output_report_json: str | Path,
) -> dict[str, Any]:
    phase2ew_report = _read_json(phase2ew_report_json)
    ...
    negative_rows = [
        ...
        if row["control_id"] != "positive_control_original_phase2ew_replay"
    ]
    checks = {
        "source_phase2ew_passed": phase2ew_report.get("passed") is True,
        "positive_control_still_passes": any(
            row["control_id"] == "positive_control_original_phase2ew_replay"
    ...
    report = {
        "artifact_family": "phase2ex_replay_bundle_summary_replay_negative_controls",
        "passed": passed,
        "ready_for_phase2ew_gate_strictness_claim": passed,
        ...
        "supported_claims": [
            (
                "the Phase2EW replay gate rejects source summary tampering, "
                "recorded check tampering, failed upstream EU validation, "
                "incomplete upstream EV negative controls, missing or malformed "
        ...
        "next_required_experiment": (
            ...
            if passed
            else "repair_phase2ex_replay_bundle_summary_replay_negative_controls"
        ),
        "evidence": {
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Audit Phase2EX replay bundle summary replay negative controls."
    )
    parser.add_argument("--phase2ew-report-json", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--output-report-json", required=True)
    parser.add_argument("--no-fail", action="store_true")
    args = parser.parse_args()
    report = audit_phase2ex_replay_bundle_summary_replay_negative_controls(
        phase2ew_report_json=args.phase2ew_report_json,

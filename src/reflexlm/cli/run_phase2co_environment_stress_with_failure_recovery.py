...
STRESS_IDS = (
    ...
    "wrong_cwd_then_subdir_recover",
    "cmd_wrapper_failure_then_recover",
)
ENV_KEY = "PHASE2CO_ENV_TOKEN"


def _payload_token(*, seed: int, repository_id: str, stress_id: str) -> str:
    digest = hashlib.sha256(
        f"phase2co|{seed}|{repository_id}|{stress_id}".encode("utf-8")
    ).hexdigest()
    return digest[:12]


def _missing_env_then_overlay_recover_episode(
    *,
    repository_id: str,
    token: str,
) -> dict[str, Any]:
    fail_code = (
        "import os, sys; "
        f"sys.stderr.write('env-missing-{token}\\n'); "
        f"raise SystemExit(0 if os.environ.get({ENV_KEY!r}) == {token!r} else 31)"
    )
    recover_code = (
        "import os; "
        f"print('env-recovered-{token}-' + os.environ[{ENV_KEY!r}])"
    )
    fail_step = {
        "action_type": "RUN_COMMAND",
        "argv": ["<PYTHON>", "-c", fail_code],
        "expected_exit_code": 31,
        "expected_stderr_contains": f"env-missing-{token}",
    }
    recover_step = {
        "action_type": "RUN_COMMAND",
        ...
        "expected_stdout_contains": f"env-recovered-{token}-{token}",
    }
    return {
        "episode_id": f"phase2co-{repository_id}-missing-env-recover-{token}",
        "description": "Missing environment token failure followed by bounded env overlay recovery.",
        ...
        "generator": {
            "phase": "phase2co",
            "stress_id": "missing_env_then_overlay_recover",
            "payload_token": token,
        },
    }


def _wrong_cwd_then_subdir_recover_episode(
    *,
    repository_id: str,
    token: str,
) -> dict[str, Any]:
    cwd = f".reflexlm_runtime_probe/phase2co-{repository_id}-{token}"
    expected_name = f"phase2co-{repository_id}-{token}"
    fail_code = (
        "from pathlib import Path; import sys; "
        f"sys.stderr.write('cwd-mismatch-{token}-' + Path.cwd().name + '\\n'); "
        f"raise SystemExit(0 if Path.cwd().name == {expected_name!r} else 32)"
    )
    recover_code = (
        "from pathlib import Path; "
        f"print('cwd-recovered-{token}-' + Path.cwd().name)"
    )
    fail_step = {
        "action_type": "RUN_COMMAND",
        "argv": ["<PYTHON>", "-c", fail_code],
        "expected_exit_code": 32,
        "expected_stderr_contains": f"cwd-mismatch-{token}",
    }
    recover_step = {
        "action_type": "RUN_COMMAND",
        "argv": ["<PYTHON>", "-c", recover_code],
        "cwd": cwd,
        "expected_exit_code": 0,
        "expected_stdout_contains": f"cwd-recovered-{token}-{expected_name}",
    }
    return {
        "episode_id": f"phase2co-{repository_id}-cwd-recover-{token}",
        "description": "Wrong working-directory failure followed by bounded cwd recovery.",
        "task_type": "common_error_recovery_routine",
        "ambient_observation_actions": ["READ_STDERR", "READ_STDOUT"],
        "permissions": [
            fail_step,
        ...
        "generator": {
            "phase": "phase2co",
            "stress_id": "wrong_cwd_then_subdir_recover",
            "payload_token": token,
            "cwd": cwd,
        },
    }
...
def _cmd_wrapper_failure_then_recover_episode(
    *,
    repository_id: str,
    token: str,
) -> dict[str, Any]:
    fail_code = (
        "import sys; "
        f"sys.stderr.write('cmd-wrapper-failure-{token}\\n'); "
        "raise SystemExit(33)"
    )
    recover_code = f"print('cmd-wrapper-recovered-{token}')"
    fail_step = {
        "action_type": "RUN_COMMAND",
        "argv": ["cmd.exe", "/d", "/c", "<PYTHON>", "-c", fail_code],
        "expected_exit_code": 33,
        "expected_stderr_contains": f"cmd-wrapper-failure-{token}",
    }
    recover_step = {
        "action_type": "RUN_COMMAND",
        "argv": ["cmd.exe", "/d", "/c", "<PYTHON>", "-c", recover_code],
        "expected_exit_code": 0,
        "expected_stdout_contains": f"cmd-wrapper-recovered-{token}",
    }
    return {
        "episode_id": f"phase2co-{repository_id}-cmd-wrapper-recover-{token}",
        "description": "Explicit cmd.exe argv wrapper failure followed by bounded wrapper recovery.",
        "task_type": "common_error_recovery_routine",
        "ambient_observation_actions": ["READ_STDERR", "READ_STDOUT"],
        "permissions": [
            fail_step,
        ...
        "generator": {
            "phase": "phase2co",
            "stress_id": "cmd_wrapper_failure_then_recover",
            "payload_token": token,
            "shell": "cmd.exe /d /c",
        },
    }
...
def _build_episode(
    *,
    ...
    if stress_id == "missing_env_then_overlay_recover":
        return _missing_env_then_overlay_recover_episode(
            repository_id=repository_id,
            token=token,
        )
    if stress_id == "wrong_cwd_then_subdir_recover":
        return _wrong_cwd_then_subdir_recover_episode(
            repository_id=repository_id,
            token=token,
        )
    if stress_id == "cmd_wrapper_failure_then_recover":
        return _cmd_wrapper_failure_then_recover_episode(
            repository_id=repository_id,
            token=token,
        )
    raise ValueError(f"unknown phase2co stress_id: {stress_id}")
...
def run_phase2co_environment_stress_with_failure_recovery(
    *,
    ...
    if not isinstance(repositories, list) or not repositories:
        raise ValueError("phase2co suite requires non-empty repositories")
    ...
    checks = {
        **identity_checks,
        ...
        "all_failure_episodes_observed_failure": total_observed_failures
        == total_episodes,
    ...
    report = {
        ...
        "supported_claims": [
            (
                ...
                "environment and explicit cmd.exe wrapper stress episodes that first "
                "observed a controlled runtime failure and then executed a bounded "
                "recovery action without free-form shell generation or unrelated expert "
                "loading"
            )
        ]
        if passed
        ...
        "next_required_experiment": (
            ...
            if passed
            else "repair_phase2co_environment_stress_with_failure_recovery"
        ),
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Phase2CO bounded environment stress recovery matrix."
    )

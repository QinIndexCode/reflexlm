...
def _phase2fo_fixture(tmp_path: Path) -> Path:
    _phase2fo_report(tmp_path)
    return tmp_path / "phase2fo.json"


def test_phase2fp_rejects_replay_bundle_summary_portability_summary_negative_controls(
    tmp_path: Path,
) -> None:
    report = audit_phase2fp_replay_bundle_summary_portability_summary_negative_controls(
        phase2fo_report_json=_phase2fo_fixture(tmp_path),

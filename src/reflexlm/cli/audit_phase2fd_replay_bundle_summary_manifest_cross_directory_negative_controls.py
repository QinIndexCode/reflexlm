...
def _copy_replay_tree(
    *,
    phase2fc_report: dict[str, Any],
    case_dir: Path,
) -> dict[str, str]:
    source_replay_dir = Path(str(phase2fc_report.get("evidence", {}).get("replay_dir", "")))
    if not source_replay_dir.exists():
        raise ValueError("Phase2FD requires a readable Phase2FC replay directory")
...
def _materialize_control_report(
    *,
    ...
    control_report = deepcopy(phase2fc_report)
    path_map = _copy_replay_tree(phase2fc_report=phase2fc_report, case_dir=case_dir)
    ...
    replay_report = _read_json(control_report["evidence"]["replayed_phase2fa_report"])
    replay_report = _rewrite_paths(replay_report, path_map)
    _write_json(control_report["evidence"]["replayed_phase2fa_report"], replay_report)
...
def _replay_report_path(report: dict[str, Any]) -> Path:
    return Path(str(report.get("evidence", {}).get("replayed_phase2fa_report", "")))
...
def _mutate_source_summary_failed(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    report.setdefault("source_summary", {})["phase2fa_passed"] = False


def _mutate_recorded_check_false(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    report.setdefault("checks", {})["replay_phase2fa_validation_passed"] = False
...
def _mutate_missing_bundle_role(report: dict[str, Any], case_dir: Path) -> None:
    ...
    report.setdefault("replay_summary", {})["bundle_artifact_roles"] = [
        ...
        if role != "replayed_phase2eu_validation"
    ]
...
CONTROL_SPECS: tuple[dict[str, Any], ...] = (
    {
        "control_id": "positive_control_original_phase2fc_replay",
        "mutation": None,
    ...
    },
    {
        ...
        "expected_failed_checks": ["source_phase2fa_and_phase2fb_passed"],
    },
    {
    ...
    },
    {
        ...
        "expected_passed": False,
        "expected_failed_checks": [
            "replayed_phase2fa_report_readable",
            "replayed_phase2fa_validation_passed",
        ],
    },
    {
    ...
    },
    {
        ...
        "expected_passed": False,
        "expected_failed_checks": ["replayed_phase2fa_validation_passed"],
    },
    {
...
def _run_control(
    *,
    ...
    control_report = _materialize_control_report(
        phase2fc_report=phase2fc_report,
        case_dir=case_dir,
    )
    mutation: Mutation | None = control_spec["mutation"]
    if mutation is not None:
        mutation(control_report, case_dir)
    control_report_json = case_dir / "phase2fc_control_report.json"
    ...
    validation_report_json = case_dir / "phase2fc_validation.json"
...
def audit_phase2fd_replay_bundle_summary_manifest_cross_directory_negative_controls(
    *,
    phase2fc_report_json: str | Path,
    output_dir: str | Path,
    output_report_json: str | Path,
) -> dict[str, Any]:
    phase2fc_report = _read_json(phase2fc_report_json)
    ...
    negative_rows = [
        ...
        if row["control_id"] != "positive_control_original_phase2fc_replay"
    ]
    checks = {
        "source_phase2fc_passed": phase2fc_report.get("passed") is True,
        "positive_control_still_passes": any(
            row["control_id"] == "positive_control_original_phase2fc_replay"
    ...
    report = {
        ...
        "passed": passed,
        "ready_for_phase2fc_gate_strictness_claim": passed,
        ...
        "supported_claims": [
            (
                "the Phase2FC cross-directory replay gate rejects source summary "
                "tampering, recorded check tampering, incomplete upstream FB "
        ...
        "next_required_experiment": (
            ...
            if passed
            else "repair_phase2fd_replay_bundle_summary_manifest_cross_directory_negative_controls"
        ),
        "evidence": {
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Audit Phase2FD replay bundle summary manifest replay negative controls."
    )
    parser.add_argument("--phase2fc-report-json", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--output-report-json", required=True)
    parser.add_argument("--no-fail", action="store_true")
    args = parser.parse_args()
    report = audit_phase2fd_replay_bundle_summary_manifest_cross_directory_negative_controls(
        phase2fc_report_json=args.phase2fc_report_json,

...
def _copy_replay_tree(
    *,
    phase2es_report: dict[str, Any],
    case_dir: Path,
) -> dict[str, str]:
    source_replay_dir = Path(str(phase2es_report.get("evidence", {}).get("replay_dir", "")))
    if not source_replay_dir.exists():
        raise ValueError("Phase2ET requires a readable Phase2ES replay directory")
    ...
    path_map: dict[str, str] = {str(source_replay_dir): str(target_replay_dir)}
...
def _materialize_control_report(
    *,
    ...
    control_report = deepcopy(phase2es_report)
    path_map = _copy_replay_tree(phase2es_report=phase2es_report, case_dir=case_dir)
    ...
    replay_report = _read_json(control_report["evidence"]["replayed_phase2eq_report"])
    replay_report = _rewrite_paths(replay_report, path_map)
    _write_json(control_report["evidence"]["replayed_phase2eq_report"], replay_report)
    replay_manifest_path = Path(
        str(replay_report.get("evidence", {}).get("reproducibility_manifest", ""))
    )
    replay_manifest = _rewrite_paths(_read_json(replay_manifest_path), path_map)
    _write_json(replay_manifest_path, replay_manifest)
    return control_report


def _replay_report_path(report: dict[str, Any]) -> Path:
    return Path(str(report.get("evidence", {}).get("replayed_phase2eq_report", "")))
...
def _mutate_source_summary_failed(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    report.setdefault("source_summary", {})["phase2eq_passed"] = False


def _mutate_recorded_check_false(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    report.setdefault("checks", {})["replay_phase2eq_validation_passed"] = False
...
def _mutate_source_negative_controls_incomplete(
    ...
    del case_dir
    report.setdefault("source_summary", {})["phase2er_negative_controls_failed"] = 0
...
def _mutate_missing_replayed_bundle_file(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    markdown = Path(str(_bundle_entry(report, "replayed_bundle_summary_markdown")["path"]))
    if markdown.exists():
        markdown.unlink()
...
def _mutate_missing_source_role(report: dict[str, Any], case_dir: Path) -> None:
    ...
    report.setdefault("replay_summary", {})["source_report_roles"] = [
        ...
        if role != "phase2em_report"
    ]


def _mutate_missing_bundle_role(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    report.setdefault("replay_summary", {})["bundle_artifact_roles"] = [
        role
        for role in report.get("replay_summary", {}).get("bundle_artifact_roles", [])
        if role != "replayed_phase2ek_validation"
    ]
...
def _mutate_control_artifact_count_collapsed(
    ...
    del case_dir
    report.setdefault("replay_summary", {})["control_artifact_count"] = 0
...
CONTROL_SPECS: tuple[dict[str, Any], ...] = (
    {
        "control_id": "positive_control_original_phase2es_replay",
        "mutation": None,
    ...
    },
    {
        ...
        "expected_failed_checks": ["source_phase2eq_and_phase2er_passed"],
    },
    {
    ...
    },
    {
        ...
        "expected_passed": False,
        "expected_failed_checks": [
            "replayed_phase2eq_report_readable",
            "replayed_phase2eq_validation_passed",
        ],
    },
    {
    ...
    },
    {
        ...
        "expected_passed": False,
        "expected_failed_checks": ["replayed_phase2eq_validation_passed"],
    },
    {
    ...
    {
        "control_id": "negative_missing_bundle_role",
        "mutation": _mutate_missing_bundle_role,
        "expected_passed": False,
        "expected_failed_checks": ["replayed_bundle_artifact_roles_complete"],
    },
    {
        "control_id": "negative_control_artifact_count_collapsed",
        "mutation": _mutate_control_artifact_count_collapsed,
        "expected_passed": False,
        "expected_failed_checks": ["replayed_control_artifact_count_preserved"],
    },
    {
...
def _run_control(
    *,
    ...
    control_report = _materialize_control_report(
        phase2es_report=phase2es_report,
        case_dir=case_dir,
    )
    mutation: Mutation | None = control_spec["mutation"]
    if mutation is not None:
        mutation(control_report, case_dir)
    control_report_json = case_dir / "phase2es_control_report.json"
    _write_json(control_report_json, control_report)
    validation = validate_phase2es_replay_bundle_summary_manifest_cross_directory_replay(
        control_report
    )
    validation_report_json = case_dir / "phase2es_validation.json"
...
def audit_phase2et_replay_bundle_summary_manifest_cross_directory_negative_controls(
    *,
    phase2es_report_json: str | Path,
    output_dir: str | Path,
    output_report_json: str | Path,
) -> dict[str, Any]:
    phase2es_report = _read_json(phase2es_report_json)
    ...
    negative_rows = [
        ...
        if row["control_id"] != "positive_control_original_phase2es_replay"
    ]
    checks = {
        "source_phase2es_passed": phase2es_report.get("passed") is True,
        "positive_control_still_passes": any(
            row["control_id"] == "positive_control_original_phase2es_replay"
    ...
    report = {
        "artifact_family": (
            "phase2et_replay_bundle_summary_manifest_cross_directory_negative_controls"
        ),
        "passed": passed,
        "ready_for_phase2es_gate_strictness_claim": passed,
        ...
        "supported_claims": [
            (
                "the Phase2ES cross-directory replay gate rejects source summary "
                "tampering, recorded check tampering, incomplete upstream ER "
                "negative controls, missing or malformed replay reports, replay "
                "manifest loss, replay artifact tampering or deletion, missing "
                "source or bundle roles, copied-control count drift, missing "
                "steps, non-distinct replay directories, collapsed hash counts, "
        ...
        "next_required_experiment": (
            ...
            if passed
            else "repair_phase2et_replay_bundle_summary_manifest_cross_directory_negative_controls"
        ),
        "evidence": {
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Audit Phase2ET replay bundle summary manifest replay negative controls."
    )
    parser.add_argument("--phase2es-report-json", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--output-report-json", required=True)
    parser.add_argument("--no-fail", action="store_true")
    args = parser.parse_args()
    report = audit_phase2et_replay_bundle_summary_manifest_cross_directory_negative_controls(
        phase2es_report_json=args.phase2es_report_json,

...
from reflexlm.cli.audit_phase2dw_replay_bundle_reproducibility_manifest import (
    ...
    validate_phase2dw_replay_bundle_reproducibility_manifest,
)
    ...
    *,
    ...
    for item in entries:
        ...
        if not source.exists():
            raise ValueError(f"Phase2DY source path missing: {source}")
...
def validate_phase2dy_replay_bundle_cross_directory_replay(
    report: dict[str, Any],
) -> dict[str, Any]:
    replay_report_path = report.get("evidence", {}).get("replayed_phase2dw_report")
    replay_validation = {}
    ...
    if replay_report_path:
        try:
            ...
            replay_validation = validate_phase2dw_replay_bundle_reproducibility_manifest(
                replay_report
            )
            ...
            replay_validation = {}
    checks = {
        "artifact_family_matches_phase2dy": (
            report.get("artifact_family")
            == "phase2dy_replay_bundle_cross_directory_replay"
        ),
        "top_level_phase2dy_passed": report.get("passed") is True,
        ...
        "source_phase2dw_and_phase2dx_passed": (
            report.get("source_summary", {}).get("phase2dw_passed") is True
            and report.get("source_summary", {}).get("phase2dx_passed") is True
        ),
        ...
        "replayed_phase2dw_report_readable": replay_report_readable,
        "replayed_phase2dw_validation_passed": replay_validation.get("passed") is True,
        "replayed_source_report_roles_complete": set(
            REQUIRED_SOURCE_REPORT_ROLES
        ).issubset(set(report.get("replay_summary", {}).get("source_report_roles", []))),
        "replayed_bundle_artifact_roles_complete": set(
            REQUIRED_ARTIFACT_ROLES
        ).issubset(set(report.get("replay_summary", {}).get("bundle_artifact_roles", []))),
...
def audit_phase2dy_replay_bundle_cross_directory_replay(
    *,
    phase2dx_report_json: str | Path,
    output_dir: str | Path,
    output_report_json: str | Path,
) -> dict[str, Any]:
    phase2dx = _read_json(phase2dx_report_json)
    phase2dw_report_json = phase2dx.get("evidence", {}).get("phase2dw_report_json")
    if not phase2dw_report_json:
        raise ValueError("Phase2DY requires Phase2DX evidence.phase2dw_report_json")
    phase2dw = _read_json(phase2dw_report_json)
    source_manifest_path = Path(
        str(phase2dw.get("evidence", {}).get("reproducibility_manifest", ""))
    )
    if not source_manifest_path.exists():
        raise ValueError("Phase2DY requires a readable Phase2DW manifest")
    ...
    replay_manifest_path = output_root / "phase2dw_replayed_manifest.json"
    _write_json(replay_manifest_path, replay_manifest)
    replay_phase2dw_report = deepcopy(phase2dw)
    replay_phase2dw_report.setdefault("evidence", {})[
        "reproducibility_manifest"
    ] = str(replay_manifest_path)
    replay_phase2dw_report_path = output_root / "phase2dw_replayed_report.json"
    _write_json(replay_phase2dw_report_path, replay_phase2dw_report)
    replay_validation = validate_phase2dw_replay_bundle_reproducibility_manifest(
        replay_phase2dw_report
    )
    replay_validation_path = output_root / "phase2dw_replayed_validation.json"
    ...
    checks = {
        ...
        "source_phase2dx_negative_controls_complete": (
            phase2dx.get("metrics", {}).get("negative_control_count")
            == phase2dx.get("metrics", {}).get("negative_controls_failed")
        ),
        "replay_manifest_written": replay_manifest_path.exists(),
        "replay_phase2dw_report_written": replay_phase2dw_report_path.exists(),
        "replay_phase2dw_validation_passed": replay_validation.get("passed") is True,
    ...
    passed = all(checks.values())
    report = {
        "artifact_family": "phase2dy_replay_bundle_cross_directory_replay",
        "passed": passed,
        ...
        "supported_claims": [
            (
                "cross-directory replay of the bounded Phase2DW reproducibility "
                "manifest with preserved source reports, bundle artifacts, hashes, "
                "reproduction steps, and boundary checks"
            )
        ]
        if passed
        else [],
        ...
        "next_required_experiment": (
            ...
            if passed
            else "repair_phase2dy_replay_bundle_cross_directory_replay"
        ),
        "evidence": {
            ...
            "replayed_phase2dw_report": str(replay_phase2dw_report_path),
            "replayed_phase2dw_validation": str(replay_validation_path),
        },
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Replay Phase2DW reproducibility manifest in a separate directory."
    )
    parser.add_argument("--phase2dx-report-json", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--output-report-json", required=True)
    parser.add_argument("--no-fail", action="store_true")
    args = parser.parse_args()
    report = audit_phase2dy_replay_bundle_cross_directory_replay(
        phase2dx_report_json=args.phase2dx_report_json,

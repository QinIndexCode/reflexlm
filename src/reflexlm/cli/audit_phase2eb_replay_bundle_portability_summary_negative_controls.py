...
def _markdown_path(report: dict[str, Any]) -> Path:
    return Path(
        str(report.get("evidence", {}).get("replay_bundle_portability_summary", ""))
    )


def _materialize_control_report(
    *,
    phase2ea_report: dict[str, Any],
    case_dir: Path,
) -> dict[str, Any]:
    control_report = deepcopy(phase2ea_report)
    source_markdown = _markdown_path(phase2ea_report)
    if not source_markdown.exists():
        raise ValueError("Phase2EB requires a readable Phase2EA markdown summary")
    target_markdown = case_dir / "phase2ea_replay_bundle_portability_summary.md"
...
def _mutate_source_summary_failed(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    report.setdefault("source_summary", {})["phase2dy_passed"] = False
...
def _mutate_phase2dy_validation_failed(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    report.setdefault("source_summary", {})["phase2dy_validation_passed"] = False


def _mutate_phase2dx_controls_incomplete(
...
def _mutate_phase2dz_controls_incomplete(
    report: dict[str, Any], case_dir: Path
) -> None:
    del case_dir
    report.setdefault("source_summary", {})["phase2dz_negative_controls_failed"] = 0
...
def _mutate_missing_required_dimension(
    ...
    if isinstance(rows, list):
        report["summary_table"]["rows"] = [
            ...
            if not (
                isinstance(row, dict)
                and row.get("Dimension") == "manifest negative controls"
            )
        ]
...
def _mutate_markdown_missing_dimension(report: dict[str, Any], case_dir: Path) -> None:
    ...
    if not isinstance(rows, list) or not rows:
        raise ValueError("Phase2EB requires summary rows for markdown dimension control")
...
def _mutate_metrics_mismatch(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
...
CONTROL_SPECS: tuple[dict[str, Any], ...] = (
    {
        "control_id": "positive_control_original_phase2ea_summary",
        "mutation": None,
    ...
    },
    {
        ...
        "expected_failed_checks": ["source_phase2dz_and_phase2dy_passed"],
    },
    {
    ...
    {
        "control_id": "negative_phase2dy_validation_failed",
        "mutation": _mutate_phase2dy_validation_failed,
        "expected_passed": False,
        "expected_failed_checks": ["source_phase2dy_validation_passed"],
    },
    {
        "control_id": "negative_phase2dx_controls_incomplete",
        "mutation": _mutate_phase2dx_controls_incomplete,
        "expected_passed": False,
        "expected_failed_checks": ["source_phase2dx_negative_controls_complete"],
    },
    {
        "control_id": "negative_phase2dz_controls_incomplete",
        "mutation": _mutate_phase2dz_controls_incomplete,
        "expected_passed": False,
        "expected_failed_checks": ["source_phase2dz_negative_controls_complete"],
    },
    {
    ...
    {
        "control_id": "negative_metrics_mismatch",
        "mutation": _mutate_metrics_mismatch,
        "expected_passed": False,
...
def _run_control(
    *,
    ...
    control_report = _materialize_control_report(
        phase2ea_report=phase2ea_report,
        case_dir=case_dir,
    )
    mutation: Mutation | None = control_spec["mutation"]
    if mutation is not None:
        mutation(control_report, case_dir)
    control_report_json = case_dir / "phase2ea_control_report.json"
    _write_json(control_report_json, control_report)
    validation = validate_phase2ea_replay_bundle_portability_summary(control_report)
    validation_report_json = case_dir / "phase2ea_validation.json"
...
def audit_phase2eb_replay_bundle_portability_summary_negative_controls(
    *,
    phase2ea_report_json: str | Path,
    output_dir: str | Path,
    output_report_json: str | Path,
) -> dict[str, Any]:
    phase2ea_report = _read_json(phase2ea_report_json)
    ...
    negative_rows = [
        ...
        if row["control_id"] != "positive_control_original_phase2ea_summary"
    ]
    checks = {
        "source_phase2ea_passed": phase2ea_report.get("passed") is True,
        "positive_control_still_passes": any(
            row["control_id"] == "positive_control_original_phase2ea_summary"
    ...
    report = {
        ...
        "passed": passed,
        "ready_for_phase2ea_gate_strictness_claim": passed,
        ...
        "supported_claims": [
            (
                "the Phase2EA replay-bundle portability-summary gate rejects "
                "source summary tampering, recorded check tampering, failed DY "
                "replay validation, incomplete DX or DZ negative controls, "
                "missing or malformed summary rows or columns, missing or weakened "
                "markdown evidence, metric mismatch, and overstated epoch claims"
            )
        ]
        if passed
        else [],
        ...
        "next_required_experiment": (
            ...
            if passed
            else "repair_phase2eb_replay_bundle_portability_summary_negative_controls"
        ),
        "evidence": {
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Audit Phase2EB replay bundle portability summary negative controls."
    )
    parser.add_argument("--phase2ea-report-json", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--output-report-json", required=True)
    parser.add_argument("--no-fail", action="store_true")
    args = parser.parse_args()
    report = audit_phase2eb_replay_bundle_portability_summary_negative_controls(
        phase2ea_report_json=args.phase2ea_report_json,

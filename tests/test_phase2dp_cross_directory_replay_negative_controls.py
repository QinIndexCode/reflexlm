...
def _phase2do_fixture(tmp_path: Path) -> Path:
    ...
    return tmp_path / "phase2do.json"


def test_phase2dp_rejects_cross_directory_replay_negative_controls(
    tmp_path: Path,
) -> None:
    report = audit_phase2dp_cross_directory_replay_negative_controls(
        phase2do_report_json=_phase2do_fixture(tmp_path),
    ...
    assert report["next_required_experiment"] == "phase2dq_replay_bundle_portability_summary"

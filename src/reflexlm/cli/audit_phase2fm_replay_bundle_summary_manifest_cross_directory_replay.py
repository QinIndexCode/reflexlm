    ...
    *,
    ...
    for index, item in enumerate(entries):
        ...
        if not source.exists():
            raise ValueError(f"Phase2FM source path missing: {source}")
...
def validate_phase2fm_replay_bundle_summary_manifest_cross_directory_replay(
    report: dict[str, Any],
) -> dict[str, Any]:
    replay_report_path = report.get("evidence", {}).get("replayed_phase2fk_report")
    replay_validation = {}
    ...
    checks = {
        ...
        "source_phase2fk_and_phase2fl_passed": (
            report.get("source_summary", {}).get("phase2fk_passed") is True
            and report.get("source_summary", {}).get("phase2fl_passed") is True
        ),
        ...
        "replayed_phase2fk_report_readable": replay_report_readable,
        "replayed_phase2fk_validation_passed": replay_validation.get("passed") is True,
        ...
        "replayed_control_artifact_count_preserved": (
            ...
            == report.get("source_summary", {}).get("phase2fk_control_artifact_count")
        ),
...
def audit_phase2fm_replay_bundle_summary_manifest_cross_directory_replay(
    *,
    phase2fl_report_json: str | Path,
    output_dir: str | Path,
    output_report_json: str | Path,
) -> dict[str, Any]:
    phase2fl = _read_json(phase2fl_report_json)
    phase2fk_report_json = phase2fl.get("evidence", {}).get("phase2fk_report_json")
    if not phase2fk_report_json:
        raise ValueError("Phase2FM requires Phase2FL evidence.phase2fk_report_json")
    phase2fk = _read_json(phase2fk_report_json)
    source_manifest_path = Path(
        str(phase2fk.get("evidence", {}).get("reproducibility_manifest", ""))
    )
    if not source_manifest_path.exists():
        raise ValueError("Phase2FM requires a readable Phase2FK manifest")
    ...
    replay_manifest_path = output_root / "phase2fk_replayed_manifest.json"
    _write_json(replay_manifest_path, replay_manifest)
    replay_phase2fk_report = deepcopy(phase2fk)
    replay_phase2fk_report.setdefault("evidence", {})[
        "reproducibility_manifest"
    ] = str(replay_manifest_path)
    replay_phase2fk_report_path = output_root / "phase2fk_replayed_report.json"
    _write_json(replay_phase2fk_report_path, replay_phase2fk_report)
    replay_validation = (
        validate_phase2fk_replay_bundle_summary_reproducibility_manifest(
            replay_phase2fk_report
        )
    )
    replay_validation_path = output_root / "phase2fk_replayed_validation.json"
    ...
    checks = {
        ...
        "replay_phase2fk_report_written": replay_phase2fk_report_path.exists(),
        "replay_phase2fk_validation_passed": replay_validation.get("passed") is True,
    ...
    passed = all(checks.values())
    report = {
        "artifact_family": "phase2fm_replay_bundle_summary_manifest_cross_directory_replay",
        "passed": passed,
        ...
        "source_summary": {
            ...
            "phase2fk_control_artifact_count": phase2fk.get("metrics", {}).get(
        ...
        "supported_claims": [
            (
                "cross-directory replay of the bounded Phase2FK reproducibility "
        ...
        "next_required_experiment": (
            ...
            if passed
            else "repair_phase2fm_replay_bundle_summary_manifest_cross_directory_replay"
        ),
        "evidence": {
            ...
            "replayed_phase2fk_report": str(replay_phase2fk_report_path),
            "replayed_phase2fk_validation": str(replay_validation_path),
        },
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Replay Phase2FK reproducibility manifest in a separate directory."
    )
    parser.add_argument("--phase2fl-report-json", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--output-report-json", required=True)
    parser.add_argument("--no-fail", action="store_true")
    args = parser.parse_args()
    report = audit_phase2fm_replay_bundle_summary_manifest_cross_directory_replay(
        phase2fl_report_json=args.phase2fl_report_json,

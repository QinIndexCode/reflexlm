...
def _artifact_by_role(report: dict[str, Any], role: str) -> dict[str, Any]:
    for item in _manifest_artifacts(report):
        if item.get("role") == role:
            return item
    raise ValueError(f"missing EO bundle artifact role: {role}")
...
def _first_control_artifact(report: dict[str, Any], role: str) -> dict[str, Any]:
    for item in _manifest_artifacts(report):
        if item.get("role") == role:
            return item
    raise ValueError(f"missing EO control artifact role: {role}")


def _materialize_control_report(
    *,
    phase2eo_report: dict[str, Any],
    case_dir: Path,
) -> dict[str, Any]:
    control_report = deepcopy(phase2eo_report)
    source_manifest_path = _manifest_path(phase2eo_report)
    if not source_manifest_path.exists():
        raise ValueError("Phase2EP requires a readable Phase2EO bundle manifest")
    source_manifest = _read_json(source_manifest_path)
    source_artifacts = source_manifest.get("artifacts", [])
    if not isinstance(source_artifacts, list):
        raise ValueError("Phase2EP requires a list-valued Phase2EO manifest.artifacts")
    ...
    for item in source_artifacts:
        ...
        if source_path is not None:
            ...
            if not source.exists():
                raise ValueError(f"Phase2EP source artifact missing: {source}")
            try:
...
def _mutate_source_summary_failed(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    report.setdefault("source_summary", {})["phase2em_passed"] = False
...
def _mutate_source_phase2em_validation_failed(
    report: dict[str, Any], case_dir: Path
) -> None:
    del case_dir
    report.setdefault("source_summary", {})["phase2em_validation_passed"] = False


def _mutate_negative_controls_incomplete(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    report.setdefault("source_summary", {})["phase2en_negative_controls_failed"] = 0
...
def _mutate_readme_missing_boundary(report: dict[str, Any], case_dir: Path) -> None:
    ...
    readme.write_text("Phase2EO replay bundle\n", encoding="utf-8")
...
CONTROL_SPECS: tuple[dict[str, Any], ...] = (
    {
        "control_id": "positive_control_original_phase2eo_bundle",
        "mutation": None,
    ...
    },
    {
        ...
        "expected_failed_checks": ["source_phase2em_and_phase2en_passed"],
    },
    {
    ...
    {
        "control_id": "negative_source_phase2em_validation_failed",
        "mutation": _mutate_source_phase2em_validation_failed,
        "expected_passed": False,
        "expected_failed_checks": ["source_phase2em_validation_passed"],
    },
    {
...
def _run_control(
    *,
    ...
    control_report = _materialize_control_report(
        phase2eo_report=phase2eo_report,
        case_dir=case_dir,
    )
    mutation: Mutation | None = control_spec["mutation"]
    if mutation is not None:
        mutation(control_report, case_dir)
    control_report_json = case_dir / "phase2eo_control_report.json"
    _write_json(control_report_json, control_report)
    validation = validate_phase2eo_replay_bundle_summary_replay_bundle(control_report)
    validation_report_json = case_dir / "phase2eo_validation.json"
...
def audit_phase2ep_replay_bundle_summary_replay_bundle_negative_controls(
    *,
    phase2eo_report_json: str | Path,
    output_dir: str | Path,
    output_report_json: str | Path,
) -> dict[str, Any]:
    phase2eo_report = _read_json(phase2eo_report_json)
    ...
    negative_rows = [
        ...
        if row["control_id"] != "positive_control_original_phase2eo_bundle"
    ]
    checks = {
        "source_phase2eo_passed": phase2eo_report.get("passed") is True,
        "positive_control_still_passes": any(
            row["control_id"] == "positive_control_original_phase2eo_bundle"
    ...
    report = {
        ...
        "passed": passed,
        "ready_for_phase2eo_gate_strictness_claim": passed,
        ...
        "supported_claims": [
            (
                "the Phase2EO replay bundle gate rejects source summary tampering, "
                "recorded check tampering, failed upstream EM validation, incomplete "
                "upstream EN negative controls, missing or malformed manifests, "
        ...
        "next_required_experiment": (
            ...
            if passed
            else "repair_phase2ep_replay_bundle_summary_replay_bundle_negative_controls"
        ),
        "evidence": {
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Audit Phase2EP replay bundle summary replay bundle negative controls."
    )
    parser.add_argument("--phase2eo-report-json", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--output-report-json", required=True)
    parser.add_argument("--no-fail", action="store_true")
    args = parser.parse_args()
    report = audit_phase2ep_replay_bundle_summary_replay_bundle_negative_controls(
        phase2eo_report_json=args.phase2eo_report_json,

...
REQUIRED_ARTIFACT_ROLES: tuple[str, ...] = (
    ...
    "replayed_phase2fy_report",
    "replayed_phase2fy_validation",
...
def _control_artifact_entries(
    *,
    ...
    artifacts = []
    rows = phase2ga.get("replayed_control_results", [])
    ...
    for index, row in enumerate(rows):
        ...
        artifacts.append(
            _copy_artifact(
                ...
                target_path=case_dir / "phase2fy_control_report.json",
        ...
        artifacts.append(
            _copy_artifact(
                ...
                target_path=case_dir / "phase2fy_validation.json",
                content_type="application/json",
            )
        )
        artifacts[-1]["control_id"] = control_id
...
def _write_readme(
    ...
    *,
    ...
    text = "\n".join(
        [
            "# Phase2GC Replay Bundle Summary Replay Bundle",
            "",
            "This bundle contains bounded cross-directory replay evidence for "
            "the Phase2FY replay-bundle summary portability summary and the "
            "Phase2GB negative-control audit of the Phase2GA replay gate.",
            "",
            "Included artifacts:",
            "- replayed Phase2FY summary Markdown",
            "- replayed Phase2FY report",
            "- replayed Phase2FY validation",
            "- replayed Phase2FZ control result summary",
            "- copied Phase2FZ control reports and validations",
            "- Phase2GA source report",
            "- Phase2GB negative-control report",
            "",
            "Boundary:",
            ...
            "",
            f"Phase2GA passed: {phase2ga.get('passed') is True}",
            f"Phase2GB passed: {phase2gb.get('passed') is True}",
            "Phase2GA replayed summary rows: "
            f"{phase2ga.get('metrics', {}).get('replayed_summary_row_count')}",
            "Phase2GB negative controls failed: "
            f"{phase2gb.get('metrics', {}).get('negative_controls_failed')}",
            "",
        ]
    )
...
def validate_phase2gc_replay_bundle_summary_replay_bundle(
    report: dict[str, Any],
) -> dict[str, Any]:
    ...
    checks = {
        "artifact_family_matches_phase2gc": (
            report.get("artifact_family")
            == "phase2gc_replay_bundle_summary_replay_bundle"
        ),
        "top_level_phase2gc_passed": report.get("passed") is True,
        ...
        "source_phase2ga_and_phase2gb_passed": (
            report.get("source_summary", {}).get("phase2ga_passed") is True
            and report.get("source_summary", {}).get("phase2gb_passed") is True
        ),
        "source_phase2ga_validation_passed": (
            report.get("source_summary", {}).get("phase2ga_validation_passed") is True
        ),
        "source_negative_controls_complete": (
            report.get("source_summary", {}).get("phase2gb_negative_control_count")
            == report.get("source_summary", {}).get("phase2gb_negative_controls_failed")
        ),
...
def audit_phase2gc_replay_bundle_summary_replay_bundle(
    *,
    ...
    phase2ga_report_json = phase2gb.get("evidence", {}).get("phase2ga_report_json")
    if not phase2ga_report_json:
        raise ValueError("Phase2GC requires Phase2GB evidence.phase2ga_report_json")
    phase2ga = _read_json(phase2ga_report_json)
    phase2ga_validation = validate_phase2ga_replay_bundle_summary_cross_directory_replay(
        phase2ga
    )
    ...
    artifacts = [
        _copy_artifact(
            ...
            source_path=phase2ga["evidence"]["replayed_markdown"],
            target_path=output_root / "phase2fy_replayed_bundle_portability_summary.md",
            content_type="text/markdown",
        ),
        _copy_artifact(
            role="replayed_phase2fy_report",
            source_path=phase2ga["evidence"]["replayed_phase2fy_report"],
            target_path=output_root / "phase2fy_replayed_report.json",
            content_type="application/json",
        ),
        _copy_artifact(
            role="replayed_phase2fy_validation",
            source_path=phase2ga["evidence"]["replayed_phase2fy_validation"],
            target_path=output_root / "phase2fy_replayed_validation.json",
            content_type="application/json",
        ),
        _copy_artifact(
            role="replayed_control_results_json",
            source_path=phase2ga["evidence"]["replayed_control_results_json"],
            target_path=output_root / "phase2fz_replayed_control_results.json",
            content_type="application/json",
        ),
        _copy_artifact(
            role="phase2ga_report",
            source_path=phase2ga_report_json,
            target_path=output_root / "phase2ga_replay_bundle_summary_cross_directory_replay.json",
            content_type="application/json",
        ),
        _copy_artifact(
            role="phase2gb_negative_control_report",
            source_path=phase2gb_report_json,
            target_path=output_root / "phase2gb_replay_bundle_summary_replay_negative_controls.json",
            content_type="application/json",
        ),
    ]
    ...
    artifacts.append(
        _write_readme(output_root / "README.md", phase2ga=phase2ga, phase2gb=phase2gb)
    )
    manifest = {
        "artifact_family": "phase2gc_replay_bundle_summary_replay_bundle_manifest",
    ...
    checks = {
        "source_phase2ga_passed": phase2ga.get("passed") is True,
        "source_phase2gb_passed": phase2gb.get("passed") is True,
        "source_phase2ga_validation_passed": phase2ga_validation.get("passed") is True,
        "source_phase2gb_negative_controls_complete": (
            phase2gb.get("metrics", {}).get("negative_control_count")
            == phase2gb.get("metrics", {}).get("negative_controls_failed")
        ),
        ...
        == phase2ga.get("metrics", {}).get("replayed_control_count", 0) * 2,
        "manifest_written": manifest_path.exists(),
    }
    passed = all(checks.values())
    report = {
        "artifact_family": "phase2gc_replay_bundle_summary_replay_bundle",
        "passed": passed,
        ...
        "metrics": {
            ...
            "phase2ga_replayed_summary_row_count": phase2ga.get("metrics", {}).get(
                "replayed_summary_row_count"
            ),
            "phase2ga_replayed_control_count": phase2ga.get("metrics", {}).get(
                "replayed_control_count"
            ),
            "phase2gb_negative_control_count": phase2gb.get("metrics", {}).get(
                "negative_control_count"
            ),
            "phase2gb_negative_controls_failed": phase2gb.get("metrics", {}).get(
                "negative_controls_failed"
            ),
        },
        "source_summary": {
            "phase2ga_passed": phase2ga.get("passed") is True,
            "phase2gb_passed": phase2gb.get("passed") is True,
            "phase2ga_validation_passed": phase2ga_validation.get("passed") is True,
            "phase2gb_negative_control_count": phase2gb.get("metrics", {}).get(
                "negative_control_count"
            ),
            "phase2gb_negative_controls_failed": phase2gb.get("metrics", {}).get(
        ...
        "supported_claims": [
            (
                "artifact-level replay bundle for the bounded Phase2GA replay-bundle "
                "summary cross-directory replay and Phase2GB replay negative-control "
                "evidence, including copied FZ control reports and validations"
            )
        ]
        if passed
        else [],
        ...
        "next_required_experiment": (
            ...
            if passed
            else "repair_phase2gc_replay_bundle_summary_replay_bundle"
        ),
        "evidence": {
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build Phase2GC replay bundle summary replay bundle."
    )
    parser.add_argument("--phase2gb-report-json", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--output-report-json", required=True)
    parser.add_argument("--no-fail", action="store_true")
    args = parser.parse_args()
    report = audit_phase2gc_replay_bundle_summary_replay_bundle(
        phase2gb_report_json=args.phase2gb_report_json,

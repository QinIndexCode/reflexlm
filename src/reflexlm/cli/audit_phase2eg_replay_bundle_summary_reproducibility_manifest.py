...
REQUIRED_SOURCE_REPORT_ROLES: tuple[str, ...] = (
    "phase2ec_report",
    "phase2ed_negative_control_report",
    "phase2ee_replay_bundle_report",
    "phase2ef_negative_control_report",
)
...
def _bundle_artifact_entries(phase2ee: dict[str, Any]) -> list[dict[str, Any]]:
    manifest = _read_json(phase2ee.get("evidence", {}).get("bundle_manifest"))
...
def _reproduction_steps(
    *,
    phase2ec: dict[str, Any],
    phase2ed: dict[str, Any],
    phase2ee: dict[str, Any],
    phase2ec_report_json: str | Path,
    phase2ed_report_json: str | Path,
    phase2ee_report_json: str | Path,
    phase2ef_report_json: str | Path,
) -> list[dict[str, Any]]:
    return [
        {
            "step_id": "phase2ec_replay_bundle_summary_cross_directory_replay",
            "module": "reflexlm.cli.audit_phase2ec_replay_bundle_summary_cross_directory_replay",
            "inputs": {"phase2eb_report_json": phase2ec.get("evidence", {}).get("phase2eb_report_json")},
            "outputs": {
                "output_report_json": str(phase2ec_report_json),
                "output_dir": phase2ec.get("evidence", {}).get("replay_dir"),
            },
        },
        {
            "step_id": "phase2ed_replay_bundle_summary_replay_negative_controls",
            "module": "reflexlm.cli.audit_phase2ed_replay_bundle_summary_replay_negative_controls",
            "inputs": {"phase2ec_report_json": str(phase2ec_report_json)},
            "outputs": {
                "output_report_json": str(phase2ed_report_json),
                "output_dir": phase2ed.get("evidence", {}).get("negative_control_output_dir"),
            },
        },
        {
            "step_id": "phase2ee_replay_bundle_summary_replay_bundle",
            "module": "reflexlm.cli.audit_phase2ee_replay_bundle_summary_replay_bundle",
            "inputs": {"phase2ed_report_json": str(phase2ed_report_json)},
            "outputs": {
                "output_report_json": str(phase2ee_report_json),
                "output_dir": phase2ee.get("evidence", {}).get("bundle_dir"),
            },
        },
        {
            "step_id": "phase2ef_replay_bundle_summary_replay_bundle_negative_controls",
            "module": "reflexlm.cli.audit_phase2ef_replay_bundle_summary_replay_bundle_negative_controls",
            "inputs": {"phase2ee_report_json": str(phase2ee_report_json)},
            "outputs": {
                "output_report_json": str(phase2ef_report_json),
            },
        },
    ]


def validate_phase2eg_replay_bundle_summary_reproducibility_manifest(
    report: dict[str, Any],
) -> dict[str, Any]:
    ...
    checks = {
        "artifact_family_matches_phase2eg": (
            report.get("artifact_family")
            == "phase2eg_replay_bundle_summary_reproducibility_manifest"
        ),
        "top_level_phase2eg_passed": report.get("passed") is True,
        ...
        "source_phase2ee_and_phase2ef_passed": (
            report.get("source_summary", {}).get("phase2ee_passed") is True
            and report.get("source_summary", {}).get("phase2ef_passed") is True
        ),
        "source_phase2ee_validation_passed": (
            report.get("source_summary", {}).get("phase2ee_validation_passed") is True
        ),
        ...
        == report.get("source_summary", {}).get("phase2ee_copied_control_artifact_count"),
...
def audit_phase2eg_replay_bundle_summary_reproducibility_manifest(
    *,
    phase2ef_report_json: str | Path,
    output_manifest_json: str | Path,
    output_report_json: str | Path,
) -> dict[str, Any]:
    phase2ef = _read_json(phase2ef_report_json)
    phase2ee_report_json = phase2ef.get("evidence", {}).get("phase2ee_report_json")
    if not phase2ee_report_json:
        raise ValueError("Phase2EG requires Phase2EF evidence.phase2ee_report_json")
    phase2ee = _read_json(phase2ee_report_json)
    phase2ed_report_json = phase2ee.get("evidence", {}).get("phase2ed_report_json")
    phase2ec_report_json = phase2ee.get("evidence", {}).get("phase2ec_report_json")
    if not phase2ed_report_json or not phase2ec_report_json:
        raise ValueError("Phase2EG requires Phase2EE source report evidence paths")
    phase2ed = _read_json(phase2ed_report_json)
    phase2ec = _read_json(phase2ec_report_json)
    phase2ee_validation = validate_phase2ee_replay_bundle_summary_replay_bundle(
        phase2ee
    )
    source_reports = [
        _file_entry(role="phase2ec_report", path=phase2ec_report_json, content_type="application/json"),
        _file_entry(
            role="phase2ed_negative_control_report",
            path=phase2ed_report_json,
            content_type="application/json",
        ),
        _file_entry(
            role="phase2ee_replay_bundle_report",
            path=phase2ee_report_json,
            content_type="application/json",
        ),
        _file_entry(
            role="phase2ef_negative_control_report",
            path=phase2ef_report_json,
            content_type="application/json",
        ),
    ]
    bundle_artifacts = _bundle_artifact_entries(phase2ee)
    reproduction_steps = _reproduction_steps(
        phase2ec=phase2ec,
        phase2ed=phase2ed,
        phase2ee=phase2ee,
        phase2ec_report_json=phase2ec_report_json,
        phase2ed_report_json=phase2ed_report_json,
        phase2ee_report_json=phase2ee_report_json,
        phase2ef_report_json=phase2ef_report_json,
    )
    manifest = {
        "artifact_family": "phase2eg_replay_bundle_summary_reproducibility_manifest",
    ...
    checks = {
        "source_phase2ef_passed": phase2ef.get("passed") is True,
        "source_phase2ee_passed": phase2ee.get("passed") is True,
        "source_phase2ee_validation_passed": phase2ee_validation.get("passed") is True,
        "source_phase2ef_negative_controls_complete": (
            phase2ef.get("metrics", {}).get("negative_control_count")
            == phase2ef.get("metrics", {}).get("negative_controls_failed")
        ),
        ...
        "control_artifact_count_preserved": _control_artifact_count(bundle_artifacts)
        == phase2ee.get("metrics", {}).get("copied_control_artifact_count"),
    ...
    report = {
        ...
        "metrics": {
            ...
            "control_artifact_count": _control_artifact_count(bundle_artifacts),
            "reproduction_step_count": len(reproduction_steps),
            "phase2ef_negative_control_count": phase2ef.get("metrics", {}).get(
                "negative_control_count"
            ),
            "phase2ef_negative_controls_failed": phase2ef.get("metrics", {}).get(
                "negative_controls_failed"
            ),
        },
        "source_summary": {
            "phase2ee_passed": phase2ee.get("passed") is True,
            "phase2ef_passed": phase2ef.get("passed") is True,
            "phase2ee_validation_passed": phase2ee_validation.get("passed") is True,
            "phase2ee_copied_control_artifact_count": phase2ee.get("metrics", {}).get(
        ...
        "supported_claims": [
            (
                "reproducibility manifest for the bounded Phase2EE replay bundle "
                "and Phase2EF negative-control evidence"
            )
        ]
        if passed
        else [],
        ...
        "next_required_experiment": (
            "phase2eh_replay_bundle_summary_manifest_negative_controls"
            if passed
            else "repair_phase2eg_replay_bundle_summary_reproducibility_manifest"
        ),
        "evidence": {
            "phase2ef_report_json": str(phase2ef_report_json),
            "phase2ee_report_json": str(phase2ee_report_json),
            "phase2ed_report_json": str(phase2ed_report_json),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build Phase2EG replay bundle summary reproducibility manifest."
    )
    parser.add_argument("--phase2ef-report-json", required=True)
    parser.add_argument("--output-manifest-json", required=True)
    parser.add_argument("--output-report-json", required=True)
    parser.add_argument("--no-fail", action="store_true")
    args = parser.parse_args()
    report = audit_phase2eg_replay_bundle_summary_reproducibility_manifest(
        phase2ef_report_json=args.phase2ef_report_json,

...
def _copy_control_evidence(
    *,
    phase2el: dict[str, Any],
    target_dir: Path,
) -> list[dict[str, Any]]:
    copied_rows = []
    target_dir.mkdir(parents=True, exist_ok=True)
    for index, row in enumerate(_control_rows(phase2el)):
...
def validate_phase2em_replay_bundle_summary_cross_directory_replay(
    report: dict[str, Any],
) -> dict[str, Any]:
    replay_report_path = report.get("evidence", {}).get("replayed_phase2ek_report")
    replay_validation = {}
    ...
    if replay_report_path:
        try:
            ...
            replay_validation = validate_phase2ek_replay_bundle_summary_portability_summary(
                replay_report
            )
    ...
    checks = {
        "artifact_family_matches_phase2em": (
            report.get("artifact_family")
            == "phase2em_replay_bundle_summary_cross_directory_replay"
        ),
        "top_level_phase2em_passed": report.get("passed") is True,
        ...
        "source_phase2ek_and_phase2el_passed": (
            report.get("source_summary", {}).get("phase2ek_passed") is True
            and report.get("source_summary", {}).get("phase2el_passed") is True
        ),
        "source_phase2ek_validation_passed": (
            report.get("source_summary", {}).get("phase2ek_validation_passed") is True
        ),
        "source_negative_controls_complete": (
            report.get("source_summary", {}).get("phase2el_negative_control_count")
            == report.get("source_summary", {}).get("phase2el_negative_controls_failed")
        ),
        "replayed_phase2ek_report_readable": replay_report_readable,
        "replayed_phase2ek_validation_passed": replay_validation.get("passed") is True,
...
def audit_phase2em_replay_bundle_summary_cross_directory_replay(
    *,
    phase2el_report_json: str | Path,
    output_dir: str | Path,
    output_report_json: str | Path,
) -> dict[str, Any]:
    phase2el = _read_json(phase2el_report_json)
    phase2ek_report_json = phase2el.get("evidence", {}).get("phase2ek_report_json")
    if not phase2ek_report_json:
        raise ValueError("Phase2EM requires Phase2EL evidence.phase2ek_report_json")
    phase2ek = _read_json(phase2ek_report_json)
    source_validation = validate_phase2ek_replay_bundle_summary_portability_summary(
        phase2ek
    )
    source_markdown_path = Path(
        str(phase2ek.get("evidence", {}).get("portability_summary_markdown", ""))
    )
    if not source_markdown_path.exists():
        raise ValueError("Phase2EM requires a readable Phase2EK markdown summary")
    output_root = Path(output_dir)
    output_root.mkdir(parents=True, exist_ok=True)
    replay_markdown_path = output_root / "phase2ek_replayed_portability_summary.md"
    shutil.copy2(source_markdown_path, replay_markdown_path)
    replay_phase2ek_report = deepcopy(phase2ek)
    replay_phase2ek_report.setdefault("evidence", {})[
        "portability_summary_markdown"
    ] = str(replay_markdown_path)
    replay_phase2ek_report_path = output_root / "phase2ek_replayed_report.json"
    _write_json(replay_phase2ek_report_path, replay_phase2ek_report)
    replay_validation = validate_phase2ek_replay_bundle_summary_portability_summary(
        replay_phase2ek_report
    )
    replay_validation_path = output_root / "phase2ek_replayed_validation.json"
    ...
    replayed_control_rows_path = output_root / "phase2el_replayed_control_results.json"
    _write_json(replayed_control_rows_path, {"control_results": replayed_control_rows})
    source_rows = phase2ek.get("summary_table", {}).get("rows", [])
    replay_rows = replay_phase2ek_report.get("summary_table", {}).get("rows", [])
    source_columns = phase2ek.get("summary_table", {}).get("columns", [])
    replay_columns = replay_phase2ek_report.get("summary_table", {}).get("columns", [])
    ...
    replayed_negative_controls_failed = sum(
        ...
        if row.get("control_id") != "positive_control_original_phase2ek_summary"
    )
    ...
    checks = {
        "source_phase2el_passed": phase2el.get("passed") is True,
        "source_phase2ek_passed": phase2ek.get("passed") is True,
        "source_phase2ek_validation_passed": source_validation.get("passed") is True,
        "source_phase2el_negative_controls_complete": (
            phase2el.get("metrics", {}).get("negative_control_count")
            == phase2el.get("metrics", {}).get("negative_controls_failed")
        ),
        "replay_phase2ek_report_written": replay_phase2ek_report_path.exists(),
        "replay_phase2ek_validation_passed": replay_validation.get("passed") is True,
        ...
        == phase2el.get("metrics", {}).get("control_count"),
        ...
        == phase2el.get("metrics", {}).get("expected_failed_check_assertions"),
        "replay_directory_is_distinct": source_markdown_path.parent.resolve()
        != replay_markdown_path.parent.resolve(),
    }
    passed = all(checks.values())
    report = {
        "artifact_family": "phase2em_replay_bundle_summary_cross_directory_replay",
        "passed": passed,
        ...
        "metrics": {
            ...
            "phase2el_negative_control_count": phase2el.get("metrics", {}).get(
                "negative_control_count"
            ),
            "phase2el_negative_controls_failed": phase2el.get("metrics", {}).get(
                "negative_controls_failed"
            ),
        },
        "source_summary": {
            "phase2ek_passed": phase2ek.get("passed") is True,
            "phase2el_passed": phase2el.get("passed") is True,
            "phase2ek_validation_passed": source_validation.get("passed") is True,
            "phase2el_negative_control_count": phase2el.get("metrics", {}).get(
                "negative_control_count"
            ),
            "phase2el_negative_controls_failed": phase2el.get("metrics", {}).get(
        ...
        "replay_summary": {
            ...
            "source_control_count": phase2el.get("metrics", {}).get("control_count"),
            "replayed_control_count": len(replayed_control_rows),
            "source_negative_controls_failed": phase2el.get("metrics", {}).get(
                "negative_controls_failed"
            ),
            "replayed_negative_controls_failed": replayed_negative_controls_failed,
            "source_expected_failed_check_assertions": phase2el.get("metrics", {}).get(
        ...
        "supported_claims": [
            (
                "cross-directory replay of the bounded Phase2EK replay-bundle "
                "summary portability summary with preserved markdown hash, summary "
                "rows, summary columns, validator pass, copied Phase2EL control "
        ...
        "next_required_experiment": (
            ...
            if passed
            else "repair_phase2em_replay_bundle_summary_cross_directory_replay"
        ),
        "evidence": {
            "phase2el_report_json": str(phase2el_report_json),
            "phase2ek_report_json": str(phase2ek_report_json),
            "source_markdown": str(source_markdown_path),
            "replay_dir": str(output_root),
            "replayed_markdown": str(replay_markdown_path),
            "replayed_phase2ek_report": str(replay_phase2ek_report_path),
            "replayed_phase2ek_validation": str(replay_validation_path),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Replay Phase2EK replay-bundle summary in a separate directory."
    )
    parser.add_argument("--phase2el-report-json", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--output-report-json", required=True)
    parser.add_argument("--no-fail", action="store_true")
    args = parser.parse_args()
    report = audit_phase2em_replay_bundle_summary_cross_directory_replay(
        phase2el_report_json=args.phase2el_report_json,

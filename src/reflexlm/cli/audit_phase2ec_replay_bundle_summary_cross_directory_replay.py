...
def _copy_control_evidence(
    *,
    phase2eb: dict[str, Any],
    target_dir: Path,
) -> list[dict[str, Any]]:
    copied_rows = []
    target_dir.mkdir(parents=True, exist_ok=True)
    for index, row in enumerate(_control_rows(phase2eb)):
...
def validate_phase2ec_replay_bundle_summary_cross_directory_replay(
    report: dict[str, Any],
) -> dict[str, Any]:
    replay_report_path = report.get("evidence", {}).get("replayed_phase2ea_report")
    replay_validation = {}
    ...
    if replay_report_path:
        try:
            ...
            replay_validation = validate_phase2ea_replay_bundle_portability_summary(
                replay_report
            )
    ...
    checks = {
        "artifact_family_matches_phase2ec": (
            report.get("artifact_family")
            == "phase2ec_replay_bundle_summary_cross_directory_replay"
        ),
        "top_level_phase2ec_passed": report.get("passed") is True,
        ...
        "source_phase2ea_and_phase2eb_passed": (
            report.get("source_summary", {}).get("phase2ea_passed") is True
            and report.get("source_summary", {}).get("phase2eb_passed") is True
        ),
        "source_phase2ea_validation_passed": (
            report.get("source_summary", {}).get("phase2ea_validation_passed") is True
        ),
        "source_negative_controls_complete": (
            report.get("source_summary", {}).get("phase2eb_negative_control_count")
            == report.get("source_summary", {}).get("phase2eb_negative_controls_failed")
        ),
        "replayed_phase2ea_report_readable": replay_report_readable,
        "replayed_phase2ea_validation_passed": replay_validation.get("passed") is True,
...
def audit_phase2ec_replay_bundle_summary_cross_directory_replay(
    *,
    phase2eb_report_json: str | Path,
    output_dir: str | Path,
    output_report_json: str | Path,
) -> dict[str, Any]:
    phase2eb = _read_json(phase2eb_report_json)
    phase2ea_report_json = phase2eb.get("evidence", {}).get("phase2ea_report_json")
    if not phase2ea_report_json:
        raise ValueError("Phase2EC requires Phase2EB evidence.phase2ea_report_json")
    phase2ea = _read_json(phase2ea_report_json)
    source_validation = validate_phase2ea_replay_bundle_portability_summary(phase2ea)
    source_markdown_path = Path(
        str(phase2ea.get("evidence", {}).get("replay_bundle_portability_summary", ""))
    )
    if not source_markdown_path.exists():
        raise ValueError("Phase2EC requires a readable Phase2EA markdown summary")
    output_root = Path(output_dir)
    output_root.mkdir(parents=True, exist_ok=True)
    replay_markdown_path = output_root / "phase2ea_replayed_bundle_portability_summary.md"
    shutil.copy2(source_markdown_path, replay_markdown_path)
    replay_phase2ea_report = deepcopy(phase2ea)
    replay_phase2ea_report.setdefault("evidence", {})[
        "replay_bundle_portability_summary"
    ] = str(replay_markdown_path)
    replay_phase2ea_report_path = output_root / "phase2ea_replayed_report.json"
    _write_json(replay_phase2ea_report_path, replay_phase2ea_report)
    replay_validation = validate_phase2ea_replay_bundle_portability_summary(
        replay_phase2ea_report
    )
    replay_validation_path = output_root / "phase2ea_replayed_validation.json"
    ...
    replayed_control_rows_path = output_root / "phase2eb_replayed_control_results.json"
    _write_json(replayed_control_rows_path, {"control_results": replayed_control_rows})
    source_rows = phase2ea.get("summary_table", {}).get("rows", [])
    replay_rows = replay_phase2ea_report.get("summary_table", {}).get("rows", [])
    source_columns = phase2ea.get("summary_table", {}).get("columns", [])
    replay_columns = replay_phase2ea_report.get("summary_table", {}).get("columns", [])
    ...
    replayed_negative_controls_failed = sum(
        ...
        if row.get("control_id") != "positive_control_original_phase2ea_summary"
    )
    ...
    checks = {
        "source_phase2eb_passed": phase2eb.get("passed") is True,
        "source_phase2ea_passed": phase2ea.get("passed") is True,
        "source_phase2ea_validation_passed": source_validation.get("passed") is True,
        "source_phase2eb_negative_controls_complete": (
            phase2eb.get("metrics", {}).get("negative_control_count")
            == phase2eb.get("metrics", {}).get("negative_controls_failed")
        ),
        "replay_phase2ea_report_written": replay_phase2ea_report_path.exists(),
        "replay_phase2ea_validation_passed": replay_validation.get("passed") is True,
        ...
        == phase2eb.get("metrics", {}).get("control_count"),
        ...
        == phase2eb.get("metrics", {}).get("expected_failed_check_assertions"),
        "replay_directory_is_distinct": source_markdown_path.parent.resolve()
        != replay_markdown_path.parent.resolve(),
    }
    passed = all(checks.values())
    report = {
        "artifact_family": "phase2ec_replay_bundle_summary_cross_directory_replay",
        "passed": passed,
        ...
        "metrics": {
            ...
            "phase2eb_negative_control_count": phase2eb.get("metrics", {}).get(
                "negative_control_count"
            ),
            "phase2eb_negative_controls_failed": phase2eb.get("metrics", {}).get(
                "negative_controls_failed"
            ),
        },
        "source_summary": {
            "phase2ea_passed": phase2ea.get("passed") is True,
            "phase2eb_passed": phase2eb.get("passed") is True,
            "phase2ea_validation_passed": source_validation.get("passed") is True,
            "phase2eb_negative_control_count": phase2eb.get("metrics", {}).get(
                "negative_control_count"
            ),
            "phase2eb_negative_controls_failed": phase2eb.get("metrics", {}).get(
        ...
        "replay_summary": {
            ...
            "source_control_count": phase2eb.get("metrics", {}).get("control_count"),
            "replayed_control_count": len(replayed_control_rows),
            "source_negative_controls_failed": phase2eb.get("metrics", {}).get(
                "negative_controls_failed"
            ),
            "replayed_negative_controls_failed": replayed_negative_controls_failed,
            "source_expected_failed_check_assertions": phase2eb.get("metrics", {}).get(
        ...
        "supported_claims": [
            (
                "cross-directory replay of the bounded Phase2EA replay-bundle "
                "portability summary with preserved markdown hash, summary rows, "
                "summary columns, validator pass, copied Phase2EB control reports, "
                "and upstream negative-control closure"
            )
        ]
        if passed
        else [],
        ...
        "next_required_experiment": (
            ...
            if passed
            else "repair_phase2ec_replay_bundle_summary_cross_directory_replay"
        ),
        "evidence": {
            "phase2eb_report_json": str(phase2eb_report_json),
            "phase2ea_report_json": str(phase2ea_report_json),
            "source_markdown": str(source_markdown_path),
            "replay_dir": str(output_root),
            "replayed_markdown": str(replay_markdown_path),
            "replayed_phase2ea_report": str(replay_phase2ea_report_path),
            "replayed_phase2ea_validation": str(replay_validation_path),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Replay Phase2EA replay-bundle portability summary in a separate directory."
    )
    parser.add_argument("--phase2eb-report-json", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--output-report-json", required=True)
    parser.add_argument("--no-fail", action="store_true")
    args = parser.parse_args()
    report = audit_phase2ec_replay_bundle_summary_cross_directory_replay(
        phase2eb_report_json=args.phase2eb_report_json,

...
def _copy_control_evidence(
    *,
    phase2fp: dict[str, Any],
    target_dir: Path,
) -> list[dict[str, Any]]:
    copied_rows = []
    target_dir.mkdir(parents=True, exist_ok=True)
    for index, row in enumerate(_control_rows(phase2fp)):
...
def validate_phase2fq_replay_bundle_summary_cross_directory_replay(
    report: dict[str, Any],
) -> dict[str, Any]:
    replay_report_path = report.get("evidence", {}).get("replayed_phase2fo_report")
    replay_validation = {}
    ...
    if replay_report_path:
        try:
            ...
            replay_validation = validate_phase2fo_replay_bundle_summary_portability_summary(
                replay_report
            )
    ...
    checks = {
        "artifact_family_matches_phase2fq": (
            report.get("artifact_family")
            == "phase2fq_replay_bundle_summary_cross_directory_replay"
        ),
        "top_level_phase2fq_passed": report.get("passed") is True,
        ...
        "source_phase2fo_and_phase2fp_passed": (
            report.get("source_summary", {}).get("phase2fo_passed") is True
            and report.get("source_summary", {}).get("phase2fp_passed") is True
        ),
        "source_phase2fo_validation_passed": (
            report.get("source_summary", {}).get("phase2fo_validation_passed") is True
        ),
        "source_negative_controls_complete": (
            report.get("source_summary", {}).get("phase2fp_negative_control_count")
            == report.get("source_summary", {}).get("phase2fp_negative_controls_failed")
        ),
        "replayed_phase2fo_report_readable": replay_report_readable,
        "replayed_phase2fo_validation_passed": replay_validation.get("passed") is True,
...
def audit_phase2fq_replay_bundle_summary_cross_directory_replay(
    *,
    phase2fp_report_json: str | Path,
    output_dir: str | Path,
    output_report_json: str | Path,
) -> dict[str, Any]:
    phase2fp = _read_json(phase2fp_report_json)
    phase2fo_report_json = phase2fp.get("evidence", {}).get("phase2fo_report_json")
    if not phase2fo_report_json:
        raise ValueError("Phase2FQ requires Phase2FP evidence.phase2fo_report_json")
    phase2fo = _read_json(phase2fo_report_json)
    source_validation = validate_phase2fo_replay_bundle_summary_portability_summary(
        phase2fo
    )
    source_markdown_path = Path(
        str(phase2fo.get("evidence", {}).get("portability_summary_markdown", ""))
    )
    if not source_markdown_path.exists():
        raise ValueError("Phase2FQ requires a readable Phase2FO markdown summary")
    output_root = Path(output_dir)
    output_root.mkdir(parents=True, exist_ok=True)
    replay_markdown_path = output_root / "phase2fo_replayed_portability_summary.md"
    shutil.copy2(source_markdown_path, replay_markdown_path)
    replay_phase2fo_report = deepcopy(phase2fo)
    replay_phase2fo_report.setdefault("evidence", {})[
        "portability_summary_markdown"
    ] = str(replay_markdown_path)
    replay_phase2fo_report_path = output_root / "phase2fo_replayed_report.json"
    _write_json(replay_phase2fo_report_path, replay_phase2fo_report)
    replay_validation = validate_phase2fo_replay_bundle_summary_portability_summary(
        replay_phase2fo_report
    )
    replay_validation_path = output_root / "phase2fo_replayed_validation.json"
    ...
    replayed_control_rows_path = output_root / "phase2fp_replayed_control_results.json"
    _write_json(replayed_control_rows_path, {"control_results": replayed_control_rows})
    source_rows = phase2fo.get("summary_table", {}).get("rows", [])
    replay_rows = replay_phase2fo_report.get("summary_table", {}).get("rows", [])
    source_columns = phase2fo.get("summary_table", {}).get("columns", [])
    replay_columns = replay_phase2fo_report.get("summary_table", {}).get("columns", [])
    ...
    replayed_negative_controls_failed = sum(
        ...
        if row.get("control_id") != "positive_control_original_phase2fo_summary"
    )
    ...
    checks = {
        "source_phase2fp_passed": phase2fp.get("passed") is True,
        "source_phase2fo_passed": phase2fo.get("passed") is True,
        "source_phase2fo_validation_passed": source_validation.get("passed") is True,
        "source_phase2fp_negative_controls_complete": (
            phase2fp.get("metrics", {}).get("negative_control_count")
            == phase2fp.get("metrics", {}).get("negative_controls_failed")
        ),
        "replay_phase2fo_report_written": replay_phase2fo_report_path.exists(),
        "replay_phase2fo_validation_passed": replay_validation.get("passed") is True,
        ...
        == phase2fp.get("metrics", {}).get("control_count"),
        ...
        == phase2fp.get("metrics", {}).get("expected_failed_check_assertions"),
        "replay_directory_is_distinct": source_markdown_path.parent.resolve()
        != replay_markdown_path.parent.resolve(),
    }
    passed = all(checks.values())
    report = {
        "artifact_family": "phase2fq_replay_bundle_summary_cross_directory_replay",
        "passed": passed,
        ...
        "metrics": {
            ...
            "phase2fp_negative_control_count": phase2fp.get("metrics", {}).get(
                "negative_control_count"
            ),
            "phase2fp_negative_controls_failed": phase2fp.get("metrics", {}).get(
                "negative_controls_failed"
            ),
        },
        "source_summary": {
            "phase2fo_passed": phase2fo.get("passed") is True,
            "phase2fp_passed": phase2fp.get("passed") is True,
            "phase2fo_validation_passed": source_validation.get("passed") is True,
            "phase2fp_negative_control_count": phase2fp.get("metrics", {}).get(
                "negative_control_count"
            ),
            "phase2fp_negative_controls_failed": phase2fp.get("metrics", {}).get(
        ...
        "replay_summary": {
            ...
            "source_control_count": phase2fp.get("metrics", {}).get("control_count"),
            "replayed_control_count": len(replayed_control_rows),
            "source_negative_controls_failed": phase2fp.get("metrics", {}).get(
                "negative_controls_failed"
            ),
            "replayed_negative_controls_failed": replayed_negative_controls_failed,
            "source_expected_failed_check_assertions": phase2fp.get("metrics", {}).get(
        ...
        "supported_claims": [
            (
                "cross-directory replay of the bounded Phase2FO replay-bundle "
                "summary portability summary with preserved markdown hash, summary "
                "rows, summary columns, validator pass, copied Phase2FP control "
        ...
        "next_required_experiment": (
            ...
            if passed
            else "repair_phase2fq_replay_bundle_summary_cross_directory_replay"
        ),
        "evidence": {
            "phase2fp_report_json": str(phase2fp_report_json),
            "phase2fo_report_json": str(phase2fo_report_json),
            "source_markdown": str(source_markdown_path),
            "replay_dir": str(output_root),
            "replayed_markdown": str(replay_markdown_path),
            "replayed_phase2fo_report": str(replay_phase2fo_report_path),
            "replayed_phase2fo_validation": str(replay_validation_path),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Replay Phase2FO replay-bundle summary in a separate directory."
    )
    parser.add_argument("--phase2fp-report-json", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--output-report-json", required=True)
    parser.add_argument("--no-fail", action="store_true")
    args = parser.parse_args()
    report = audit_phase2fq_replay_bundle_summary_cross_directory_replay(
        phase2fp_report_json=args.phase2fp_report_json,

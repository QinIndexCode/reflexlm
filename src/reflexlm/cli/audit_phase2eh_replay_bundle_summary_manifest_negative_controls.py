...
from reflexlm.cli.audit_phase2eg_replay_bundle_summary_reproducibility_manifest import (
    validate_phase2eg_replay_bundle_summary_reproducibility_manifest,
)
...
def _entry_by_role(report: dict[str, Any], key: str, role: str) -> dict[str, Any]:
    for item in _entries(_read_manifest(report), key):
        if item.get("role") == role:
            return item
    raise ValueError(f"missing EG manifest {key} role: {role}")


def _first_control_artifact(report: dict[str, Any]) -> dict[str, Any]:
    for item in _entries(_read_manifest(report), "bundle_artifacts"):
        if item.get("role") == "replayed_control_validation":
            return item
    raise ValueError("missing EG control artifact")
    ...
    *,
    ...
    for index, item in enumerate(entries):
        ...
        if not source.exists():
            raise ValueError(f"Phase2EH source path missing: {source}")
        relative = Path(f"e{index:03d}_{source.name}")
...
def _materialize_control_report(
    *,
    phase2eg_report: dict[str, Any],
    case_dir: Path,
) -> dict[str, Any]:
    control_report = deepcopy(phase2eg_report)
    source_manifest_path = _manifest_path(phase2eg_report)
    if not source_manifest_path.exists():
        raise ValueError("Phase2EH requires a readable Phase2EG manifest")
    ...
    control_manifest["source_reports"] = _copy_entries(
        ...
        target_dir=manifest_dir / "source_reports",
    )
    control_manifest["bundle_artifacts"] = _copy_entries(
        _entries(source_manifest, "bundle_artifacts"),
        target_dir=manifest_dir / "bundle_artifacts",
        source_root=Path(
            str(
                phase2eg_report.get("source_summary", {}).get(
                    "phase2ee_bundle_dir",
    ...
    target_manifest_path = manifest_dir / "phase2eg_manifest.json"
...
def _mutate_source_summary_failed(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    report.setdefault("source_summary", {})["phase2ee_passed"] = False
...
def _mutate_source_phase2ee_validation_failed(
    report: dict[str, Any], case_dir: Path
) -> None:
    del case_dir
    report.setdefault("source_summary", {})["phase2ee_validation_passed"] = False
...
def _mutate_source_reports_not_list(report: dict[str, Any], case_dir: Path) -> None:
    ...
    manifest["source_reports"] = {"role": "phase2ec_report"}
...
def _mutate_missing_source_report_role(report: dict[str, Any], case_dir: Path) -> None:
    ...
    manifest["source_reports"] = [
        ...
        if item.get("role") != "phase2ec_report"
    ]
    _write_manifest(report, manifest)
...
def _mutate_missing_source_report_file(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    source = Path(str(_entry_by_role(report, "source_reports", "phase2ec_report")["path"]))
    if source.exists():
        source.unlink()
...
def _mutate_bundle_artifacts_not_list(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    manifest = _read_manifest(report)
    manifest["bundle_artifacts"] = {"role": "replayed_bundle_summary_markdown"}
    _write_manifest(report, manifest)
...
def _mutate_missing_bundle_artifact_role(
    ...
    manifest["bundle_artifacts"] = [
        ...
        if item.get("role") != "replayed_phase2ea_validation"
    ]
    _write_manifest(report, manifest)
...
def _mutate_missing_bundle_artifact_file(
    ...
    del case_dir
    artifact = Path(
        str(_entry_by_role(report, "bundle_artifacts", "replayed_bundle_summary_markdown")["path"])
    )
    if artifact.exists():
        artifact.unlink()
...
def _mutate_tampered_bundle_artifact_hash(
    ...
    del case_dir
    artifact = Path(str(_first_control_artifact(report)["path"]))
...
def _mutate_control_artifact_count_collapsed(
    ...
    del case_dir
    report.setdefault("source_summary", {})["phase2ee_copied_control_artifact_count"] = 0
...
CONTROL_SPECS: tuple[dict[str, Any], ...] = (
    {
        "control_id": "positive_control_original_phase2eg_manifest",
        "mutation": None,
    ...
    },
    {
        ...
        "expected_failed_checks": ["source_phase2ee_and_phase2ef_passed"],
    },
    {
    ...
    {
        "control_id": "negative_source_phase2ee_validation_failed",
        "mutation": _mutate_source_phase2ee_validation_failed,
        "expected_passed": False,
        "expected_failed_checks": ["source_phase2ee_validation_passed"],
    },
    {
...
def _run_control(
    *,
    ...
    control_report = _materialize_control_report(
        phase2eg_report=phase2eg_report,
        case_dir=case_dir,
    )
    mutation: Mutation | None = control_spec["mutation"]
    if mutation is not None:
        mutation(control_report, case_dir)
    control_report_json = case_dir / "phase2eg_control_report.json"
    _write_json(control_report_json, control_report)
    validation = validate_phase2eg_replay_bundle_summary_reproducibility_manifest(
        control_report
    )
    validation_report_json = case_dir / "phase2eg_validation.json"
...
def audit_phase2eh_replay_bundle_summary_manifest_negative_controls(
    *,
    phase2eg_report_json: str | Path,
    output_dir: str | Path,
    output_report_json: str | Path,
) -> dict[str, Any]:
    phase2eg_report = _read_json(phase2eg_report_json)
    ...
    negative_rows = [
        ...
        if row["control_id"] != "positive_control_original_phase2eg_manifest"
    ]
    checks = {
        "source_phase2eg_passed": phase2eg_report.get("passed") is True,
        "positive_control_still_passes": any(
            row["control_id"] == "positive_control_original_phase2eg_manifest"
    ...
    report = {
        "artifact_family": "phase2eh_replay_bundle_summary_manifest_negative_controls",
        "passed": passed,
        "ready_for_phase2eg_gate_strictness_claim": passed,
        ...
        "supported_claims": [
            (
                "the Phase2EG manifest gate rejects source summary tampering, "
                "recorded check tampering, failed upstream EE validation, missing "
                "or malformed manifests, missing or tampered source reports, "
                "missing or tampered bundle artifacts, copied-control count drift, "
        ...
        "next_required_experiment": (
            ...
            if passed
            else "repair_phase2eh_replay_bundle_summary_manifest_negative_controls"
        ),
        "evidence": {
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Audit Phase2EH replay bundle summary manifest negative controls."
    )
    parser.add_argument("--phase2eg-report-json", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--output-report-json", required=True)
    parser.add_argument("--no-fail", action="store_true")
    args = parser.parse_args()
    report = audit_phase2eh_replay_bundle_summary_manifest_negative_controls(
        phase2eg_report_json=args.phase2eg_report_json,

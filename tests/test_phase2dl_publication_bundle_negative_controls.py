...
def _phase2dk_fixture(tmp_path: Path) -> Path:
    ...
    return tmp_path / "phase2dk.json"


def test_phase2dl_rejects_publication_bundle_negative_controls(
    tmp_path: Path,
) -> None:
    report = audit_phase2dl_publication_bundle_negative_controls(
        phase2dk_report_json=_phase2dk_fixture(tmp_path),

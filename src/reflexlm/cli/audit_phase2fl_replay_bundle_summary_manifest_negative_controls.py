...
from reflexlm.cli.audit_phase2fk_replay_bundle_summary_reproducibility_manifest import (
    validate_phase2fk_replay_bundle_summary_reproducibility_manifest,
)
...
def _entry_by_role(report: dict[str, Any], key: str, role: str) -> dict[str, Any]:
    for item in _entries(_read_manifest(report), key):
        if item.get("role") == role:
            return item
    raise ValueError(f"missing FK manifest {key} role: {role}")
...
def _first_control_artifact(report: dict[str, Any]) -> dict[str, Any]:
    for item in _entries(_read_manifest(report), "bundle_artifacts"):
        if item.get("role") == "replayed_control_validation":
            return item
    raise ValueError("missing FK control artifact")
...
def _copy_entries(entries: list[dict[str, Any]], *, target_dir: Path) -> list[dict[str, Any]]:
    ...
    for index, item in enumerate(entries):
        ...
        if not source.exists():
            raise ValueError(f"Phase2FL source path missing: {source}")
...
def _materialize_control_report(
    *,
    phase2fk_report: dict[str, Any],
    case_dir: Path,
) -> dict[str, Any]:
    control_report = deepcopy(phase2fk_report)
    source_manifest_path = _manifest_path(phase2fk_report)
    if not source_manifest_path.exists():
        raise ValueError("Phase2FL requires a readable Phase2FK manifest")
    ...
    target_manifest_path = manifest_dir / "phase2fk_manifest.json"
...
def _mutate_source_summary_failed(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    report.setdefault("source_summary", {})["phase2fi_passed"] = False
...
def _mutate_source_phase2fi_validation_failed(
    report: dict[str, Any], case_dir: Path
) -> None:
    del case_dir
    report.setdefault("source_summary", {})["phase2fi_validation_passed"] = False
...
def _mutate_source_reports_not_list(report: dict[str, Any], case_dir: Path) -> None:
    ...
    manifest["source_reports"] = {"role": "phase2fg_report"}
...
def _mutate_missing_source_report_role(report: dict[str, Any], case_dir: Path) -> None:
    ...
    manifest["source_reports"] = [
        ...
        if item.get("role") != "phase2fg_report"
    ]
    _write_manifest(report, manifest)
...
def _mutate_missing_source_report_file(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    source = Path(str(_entry_by_role(report, "source_reports", "phase2fg_report")["path"]))
    if source.exists():
        source.unlink()
...
def _mutate_missing_bundle_artifact_role(
    ...
    manifest["bundle_artifacts"] = [
        ...
        if item.get("role") != "replayed_phase2fe_validation"
    ]
    _write_manifest(report, manifest)
...
def _mutate_control_artifact_count_collapsed(
    ...
    del case_dir
    report.setdefault("source_summary", {})["phase2fi_copied_control_artifact_count"] = 0
...
CONTROL_SPECS: tuple[dict[str, Any], ...] = (
    {
        "control_id": "positive_control_original_phase2fk_manifest",
        "mutation": None,
    ...
    },
    {
        ...
        "expected_failed_checks": ["source_phase2fi_and_phase2fj_passed"],
    },
    {
    ...
    {
        "control_id": "negative_source_phase2fi_validation_failed",
        "mutation": _mutate_source_phase2fi_validation_failed,
        "expected_passed": False,
        "expected_failed_checks": ["source_phase2fi_validation_passed"],
    },
    {
...
def _run_control(
    *,
    ...
    control_report = _materialize_control_report(
        phase2fk_report=phase2fk_report,
        case_dir=case_dir,
    )
    mutation: Mutation | None = control_spec["mutation"]
    if mutation is not None:
        mutation(control_report, case_dir)
    control_report_json = case_dir / "phase2fk_control_report.json"
    _write_json(control_report_json, control_report)
    validation = validate_phase2fk_replay_bundle_summary_reproducibility_manifest(
        control_report
    )
    validation_report_json = case_dir / "phase2fk_validation.json"
...
def audit_phase2fl_replay_bundle_summary_manifest_negative_controls(
    *,
    phase2fk_report_json: str | Path,
    output_dir: str | Path,
    output_report_json: str | Path,
) -> dict[str, Any]:
    phase2fk_report = _read_json(phase2fk_report_json)
    ...
    negative_rows = [
        ...
        if row["control_id"] != "positive_control_original_phase2fk_manifest"
    ]
    checks = {
        "source_phase2fk_passed": phase2fk_report.get("passed") is True,
        "positive_control_still_passes": any(
            row["control_id"] == "positive_control_original_phase2fk_manifest"
    ...
    report = {
        "artifact_family": "phase2fl_replay_bundle_summary_manifest_negative_controls",
        "passed": passed,
        "ready_for_phase2fk_gate_strictness_claim": passed,
        ...
        "supported_claims": [
            (
                "the Phase2FK manifest gate rejects source summary tampering, "
                "recorded check tampering, failed upstream FI validation, missing "
        ...
        "next_required_experiment": (
            ...
            if passed
            else "repair_phase2fl_replay_bundle_summary_manifest_negative_controls"
        ),
        "evidence": {
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Audit Phase2FL replay bundle summary manifest negative controls."
    )
    parser.add_argument("--phase2fk-report-json", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--output-report-json", required=True)
    parser.add_argument("--no-fail", action="store_true")
    args = parser.parse_args()
    report = audit_phase2fl_replay_bundle_summary_manifest_negative_controls(
        phase2fk_report_json=args.phase2fk_report_json,

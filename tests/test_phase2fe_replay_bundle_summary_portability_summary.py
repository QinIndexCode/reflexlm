...
from reflexlm.cli.audit_phase2fe_replay_bundle_summary_portability_summary import (
    REQUIRED_DIMENSIONS,
    SUMMARY_COLUMNS,
    audit_phase2fe_replay_bundle_summary_portability_summary,
...
def _phase2fd_fixture(tmp_path: Path) -> Path:
    _phase2fc_report(tmp_path)
    phase2fd = audit_phase2fd_replay_bundle_summary_manifest_cross_directory_negative_controls(
        phase2fc_report_json=tmp_path / "phase2fc.json",
        output_dir=tmp_path / "fd_controls",
        output_report_json=tmp_path / "phase2fd.json",
    )
    assert phase2fd["passed"] is True
    return tmp_path / "phase2fd.json"


def _phase2fe_report(tmp_path: Path) -> dict:
    report = audit_phase2fe_replay_bundle_summary_portability_summary(
        phase2fd_report_json=_phase2fd_fixture(tmp_path),
        output_report_json=tmp_path / "phase2fe.json",
        output_markdown=tmp_path / "phase2fe.md",
    )
    assert report["passed"] is True
    return report


def test_phase2fe_accepts_replay_bundle_summary_portability_summary(
    tmp_path: Path,
) -> None:
    report = _phase2fe_report(tmp_path)
    validation = validate_phase2fe_replay_bundle_summary_portability_summary(report)
    ...
    assert report["metrics"]["phase2fd_negative_control_count"] == report["metrics"][
        "phase2fd_negative_controls_failed"
    ]
...
def test_phase2fe_validation_rejects_missing_markdown(tmp_path: Path) -> None:
    report = _phase2fe_report(tmp_path)
...
def test_phase2fe_validation_rejects_missing_dimension_row(tmp_path: Path) -> None:
    report = _phase2fe_report(tmp_path)
...
def test_phase2fe_validation_rejects_markdown_missing_dimension(
    tmp_path: Path,
) -> None:
    report = _phase2fe_report(tmp_path)
...
def test_phase2fe_validation_rejects_missing_boundary(tmp_path: Path) -> None:
    report = _phase2fe_report(tmp_path)
    markdown = Path(report["evidence"]["portability_summary_markdown"])
    markdown.write_text("Phase2FE summary\n", encoding="utf-8")
    validation = validate_phase2fe_replay_bundle_summary_portability_summary(report)
...
def test_phase2fe_validation_rejects_phase2fb_controls_incomplete(
    tmp_path: Path,
) -> None:
    report = _phase2fe_report(tmp_path)
    report["source_summary"]["phase2fb_negative_controls_failed"] = 0
    validation = validate_phase2fe_replay_bundle_summary_portability_summary(report)

    assert validation["passed"] is False
    assert validation["checks"]["source_phase2fb_negative_controls_complete"] is False


def test_phase2fe_validation_rejects_phase2fd_controls_incomplete(
    tmp_path: Path,
) -> None:
    report = _phase2fe_report(tmp_path)
    report["source_summary"]["phase2fd_negative_controls_failed"] = 0
    validation = validate_phase2fe_replay_bundle_summary_portability_summary(report)

    assert validation["passed"] is False
    assert validation["checks"]["source_phase2fd_negative_controls_complete"] is False


def test_phase2fe_validation_rejects_source_validation_failure(
    tmp_path: Path,
) -> None:
    report = _phase2fe_report(tmp_path)
    report["source_summary"]["phase2fc_validation_passed"] = False
    validation = validate_phase2fe_replay_bundle_summary_portability_summary(report)

    assert validation["passed"] is False
    assert validation["checks"]["source_phase2fc_validation_passed"] is False


def test_phase2fe_validation_rejects_metrics_mismatch(tmp_path: Path) -> None:
    report = _phase2fe_report(tmp_path)
...
def test_phase2fe_validation_rejects_top_level_epoch_claim(tmp_path: Path) -> None:
    report = _phase2fe_report(tmp_path)

    ...
    *,
    ...
    for index in range(3):
        ...
        subreport_path = _write(
            ...
            _subreport(observed_recovery=observed_recovery),
        )
        repos.append(
            {
...
def _write_runtime_reports(tmp_path: Path, *, versions: list[str] | None = None) -> list[Path]:
    version_rows = versions or ["3.13.2", "3.12.10", "3.12.10"]
    runtime_rows = [
...
def test_phase2cp_audit_accepts_cross_runtime_failure_recovery_matrix(
    tmp_path: Path,
) -> None:
    runtime_reports = _write_runtime_reports(tmp_path)
...
def test_phase2cp_audit_rejects_missing_runtime_version_diversity(
    tmp_path: Path,
) -> None:
    runtime_reports = _write_runtime_reports(
        tmp_path,
        versions=["3.13.2", "3.13.2", "3.13.2"],
    )
...
def test_phase2cp_audit_rejects_missing_observed_recovery(tmp_path: Path) -> None:
    runtime_reports = _write_runtime_reports(tmp_path)
    broken = _runtime_report(
        tmp_path,
        runtime="D:\\broken\\python.exe",
        version="3.12.10",
        label="runtime-broken",
        observed_recovery=False,
    )
    runtime_reports[1] = _write(tmp_path / "runtime-1.json", broken)
    ...
    assert report["passed"] is False
    assert report["checks"]["all_report_manifests_pass_phase2co_shape"] is False

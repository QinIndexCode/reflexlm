...
from reflexlm.cli.audit_phase2gc_replay_bundle_summary_replay_bundle import (
    ...
    audit_phase2gc_replay_bundle_summary_replay_bundle,
...
def _phase2gb_fixture(tmp_path: Path) -> Path:
    _phase2ga_report(tmp_path)
    phase2gb = audit_phase2gb_replay_bundle_summary_replay_negative_controls(
        phase2ga_report_json=tmp_path / "phase2ga.json",
        output_dir=tmp_path / "gb_controls",
        output_report_json=tmp_path / "phase2gb.json",
    )
    assert phase2gb["passed"] is True
    return tmp_path / "phase2gb.json"


def _phase2gc_report(tmp_path: Path) -> dict:
    report = audit_phase2gc_replay_bundle_summary_replay_bundle(
        phase2gb_report_json=_phase2gb_fixture(tmp_path),
        output_dir=tmp_path / "gc_bundle",
        output_report_json=tmp_path / "phase2gc.json",
    )
    assert report["passed"] is True
    return report


def test_phase2gc_accepts_and_rejects_replay_bundle_summary_replay_bundle(
    tmp_path: Path,
) -> None:
    report = _phase2gc_report(tmp_path)
    validation = validate_phase2gc_replay_bundle_summary_replay_bundle(report)
    manifest = json.loads(
    ...
    assert report["metrics"]["copied_control_artifact_count"] == report["metrics"][
        "phase2ga_replayed_control_count"
    ] * 2
    assert report["metrics"]["phase2gb_negative_controls_failed"] == report[
        "metrics"
    ]["phase2gb_negative_control_count"]
    ...
    validation = validate_phase2gc_replay_bundle_summary_replay_bundle(tampered)
    ...
    manifest["artifacts"] = [
        ...
        if item["role"] != "replayed_phase2fy_validation"
    ]
    manifest_path.write_text(json.dumps(manifest), encoding="utf-8")
    validation = validate_phase2gc_replay_bundle_summary_replay_bundle(missing_role)
    assert validation["passed"] is False
    assert validation["checks"]["manifest_roles_complete"] is False

    incomplete = _isolated_bundle_report(report, tmp_path, "incomplete")
    incomplete["source_summary"]["phase2gb_negative_controls_failed"] = 0
    validation = validate_phase2gc_replay_bundle_summary_replay_bundle(incomplete)
    assert validation["passed"] is False
    ...
    validation = validate_phase2gc_replay_bundle_summary_replay_bundle(overclaim)
    assert validation["passed"] is False

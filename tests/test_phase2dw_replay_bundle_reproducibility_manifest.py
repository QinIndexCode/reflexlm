...
def _phase2dv_fixture(tmp_path: Path) -> Path:
    ...
    return tmp_path / "phase2dv.json"


def _phase2dw_report(tmp_path: Path) -> dict:
    report = audit_phase2dw_replay_bundle_reproducibility_manifest(
        phase2dv_report_json=_phase2dv_fixture(tmp_path),
...
def test_phase2dw_accepts_replay_bundle_reproducibility_manifest(
    tmp_path: Path,
) -> None:
    report = _phase2dw_report(tmp_path)
    validation = validate_phase2dw_replay_bundle_reproducibility_manifest(report)
    manifest = json.loads(
    ...
    assert report["metrics"]["bundle_artifact_count"] == 6
    assert report["metrics"]["bundle_artifact_hash_match_count"] == 6
...
def test_phase2dw_validation_rejects_tampered_bundle_artifact(tmp_path: Path) -> None:
    report = _phase2dw_report(tmp_path)
    manifest = json.loads(
        Path(report["evidence"]["reproducibility_manifest"]).read_text(encoding="utf-8")
    )
...
def test_phase2dw_validation_rejects_missing_source_report_role(
    tmp_path: Path,
) -> None:
    report = _phase2dw_report(tmp_path)
...
def test_phase2dw_validation_rejects_missing_reproduction_step(
    tmp_path: Path,
) -> None:
    report = _phase2dw_report(tmp_path)
    manifest_path = Path(report["evidence"]["reproducibility_manifest"])
...
def test_phase2dw_validation_rejects_top_level_epoch_claim(tmp_path: Path) -> None:
    report = _phase2dw_report(tmp_path)

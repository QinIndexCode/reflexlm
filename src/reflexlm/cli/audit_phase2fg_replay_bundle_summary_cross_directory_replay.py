...
def _copy_control_evidence(
    *,
    phase2ff: dict[str, Any],
    target_dir: Path,
) -> list[dict[str, Any]]:
    copied_rows = []
    target_dir.mkdir(parents=True, exist_ok=True)
    for index, row in enumerate(_control_rows(phase2ff)):
...
def validate_phase2fg_replay_bundle_summary_cross_directory_replay(
    report: dict[str, Any],
) -> dict[str, Any]:
    replay_report_path = report.get("evidence", {}).get("replayed_phase2fe_report")
    replay_validation = {}
    ...
    if replay_report_path:
        try:
            ...
            replay_validation = validate_phase2fe_replay_bundle_summary_portability_summary(
                replay_report
            )
    ...
    checks = {
        "artifact_family_matches_phase2fg": (
            report.get("artifact_family")
            == "phase2fg_replay_bundle_summary_cross_directory_replay"
        ),
        "top_level_phase2fg_passed": report.get("passed") is True,
        ...
        "source_phase2fe_and_phase2ff_passed": (
            report.get("source_summary", {}).get("phase2fe_passed") is True
            and report.get("source_summary", {}).get("phase2ff_passed") is True
        ),
        "source_phase2fe_validation_passed": (
            report.get("source_summary", {}).get("phase2fe_validation_passed") is True
        ),
        "source_negative_controls_complete": (
            report.get("source_summary", {}).get("phase2ff_negative_control_count")
            == report.get("source_summary", {}).get("phase2ff_negative_controls_failed")
        ),
        "replayed_phase2fe_report_readable": replay_report_readable,
        "replayed_phase2fe_validation_passed": replay_validation.get("passed") is True,
...
def audit_phase2fg_replay_bundle_summary_cross_directory_replay(
    *,
    phase2ff_report_json: str | Path,
    output_dir: str | Path,
    output_report_json: str | Path,
) -> dict[str, Any]:
    phase2ff = _read_json(phase2ff_report_json)
    phase2fe_report_json = phase2ff.get("evidence", {}).get("phase2fe_report_json")
    if not phase2fe_report_json:
        raise ValueError("Phase2FG requires Phase2FF evidence.phase2fe_report_json")
    phase2fe = _read_json(phase2fe_report_json)
    source_validation = validate_phase2fe_replay_bundle_summary_portability_summary(
        phase2fe
    )
    source_markdown_path = Path(
        str(phase2fe.get("evidence", {}).get("portability_summary_markdown", ""))
    )
    if not source_markdown_path.exists():
        raise ValueError("Phase2FG requires a readable Phase2FE markdown summary")
    output_root = Path(output_dir)
    output_root.mkdir(parents=True, exist_ok=True)
    replay_markdown_path = output_root / "phase2fe_replayed_portability_summary.md"
    shutil.copy2(source_markdown_path, replay_markdown_path)
    replay_phase2fe_report = deepcopy(phase2fe)
    replay_phase2fe_report.setdefault("evidence", {})[
        "portability_summary_markdown"
    ] = str(replay_markdown_path)
    replay_phase2fe_report_path = output_root / "phase2fe_replayed_report.json"
    _write_json(replay_phase2fe_report_path, replay_phase2fe_report)
    replay_validation = validate_phase2fe_replay_bundle_summary_portability_summary(
        replay_phase2fe_report
    )
    replay_validation_path = output_root / "phase2fe_replayed_validation.json"
    ...
    replayed_control_rows_path = output_root / "phase2ff_replayed_control_results.json"
    _write_json(replayed_control_rows_path, {"control_results": replayed_control_rows})
    source_rows = phase2fe.get("summary_table", {}).get("rows", [])
    replay_rows = replay_phase2fe_report.get("summary_table", {}).get("rows", [])
    source_columns = phase2fe.get("summary_table", {}).get("columns", [])
    replay_columns = replay_phase2fe_report.get("summary_table", {}).get("columns", [])
    ...
    replayed_negative_controls_failed = sum(
        ...
        if row.get("control_id") != "positive_control_original_phase2fe_summary"
    )
    ...
    checks = {
        "source_phase2ff_passed": phase2ff.get("passed") is True,
        "source_phase2fe_passed": phase2fe.get("passed") is True,
        "source_phase2fe_validation_passed": source_validation.get("passed") is True,
        "source_phase2ff_negative_controls_complete": (
            phase2ff.get("metrics", {}).get("negative_control_count")
            == phase2ff.get("metrics", {}).get("negative_controls_failed")
        ),
        "replay_phase2fe_report_written": replay_phase2fe_report_path.exists(),
        "replay_phase2fe_validation_passed": replay_validation.get("passed") is True,
        ...
        == phase2ff.get("metrics", {}).get("control_count"),
        ...
        == phase2ff.get("metrics", {}).get("expected_failed_check_assertions"),
        "replay_directory_is_distinct": source_markdown_path.parent.resolve()
        != replay_markdown_path.parent.resolve(),
    }
    passed = all(checks.values())
    report = {
        "artifact_family": "phase2fg_replay_bundle_summary_cross_directory_replay",
        "passed": passed,
        ...
        "metrics": {
            ...
            "phase2ff_negative_control_count": phase2ff.get("metrics", {}).get(
                "negative_control_count"
            ),
            "phase2ff_negative_controls_failed": phase2ff.get("metrics", {}).get(
                "negative_controls_failed"
            ),
        },
        "source_summary": {
            "phase2fe_passed": phase2fe.get("passed") is True,
            "phase2ff_passed": phase2ff.get("passed") is True,
            "phase2fe_validation_passed": source_validation.get("passed") is True,
            "phase2ff_negative_control_count": phase2ff.get("metrics", {}).get(
                "negative_control_count"
            ),
            "phase2ff_negative_controls_failed": phase2ff.get("metrics", {}).get(
        ...
        "replay_summary": {
            ...
            "source_control_count": phase2ff.get("metrics", {}).get("control_count"),
            "replayed_control_count": len(replayed_control_rows),
            "source_negative_controls_failed": phase2ff.get("metrics", {}).get(
                "negative_controls_failed"
            ),
            "replayed_negative_controls_failed": replayed_negative_controls_failed,
            "source_expected_failed_check_assertions": phase2ff.get("metrics", {}).get(
        ...
        "supported_claims": [
            (
                "cross-directory replay of the bounded Phase2FE replay-bundle "
                "summary portability summary with preserved markdown hash, summary "
                "rows, summary columns, validator pass, copied Phase2FF control "
        ...
        "next_required_experiment": (
            ...
            if passed
            else "repair_phase2fg_replay_bundle_summary_cross_directory_replay"
        ),
        "evidence": {
            "phase2ff_report_json": str(phase2ff_report_json),
            ...
            "replayed_phase2fe_report": str(replay_phase2fe_report_path),
            "replayed_phase2fe_validation": str(replay_validation_path),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Replay Phase2FE replay-bundle summary in a separate directory."
    )
    parser.add_argument("--phase2ff-report-json", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--output-report-json", required=True)
    parser.add_argument("--no-fail", action="store_true")
    args = parser.parse_args()
    report = audit_phase2fg_replay_bundle_summary_cross_directory_replay(
        phase2ff_report_json=args.phase2ff_report_json,

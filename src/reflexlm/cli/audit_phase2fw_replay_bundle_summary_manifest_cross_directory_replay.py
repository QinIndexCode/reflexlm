    ...
    *,
    ...
    for index, item in enumerate(entries):
        ...
        if not source.exists():
            raise ValueError(f"Phase2FW source path missing: {source}")
...
def validate_phase2fw_replay_bundle_summary_manifest_cross_directory_replay(
    report: dict[str, Any],
) -> dict[str, Any]:
    replay_report_path = report.get("evidence", {}).get("replayed_phase2fu_report")
    replay_validation = {}
    ...
    checks = {
        ...
        "source_phase2fu_and_phase2fv_passed": (
            report.get("source_summary", {}).get("phase2fu_passed") is True
            and report.get("source_summary", {}).get("phase2fv_passed") is True
        ),
        ...
        "replayed_phase2fu_report_readable": replay_report_readable,
        "replayed_phase2fu_validation_passed": replay_validation.get("passed") is True,
        ...
        "replayed_control_artifact_count_preserved": (
            ...
            == report.get("source_summary", {}).get("phase2fu_control_artifact_count")
        ),
...
def audit_phase2fw_replay_bundle_summary_manifest_cross_directory_replay(
    *,
    phase2fv_report_json: str | Path,
    output_dir: str | Path,
    output_report_json: str | Path,
) -> dict[str, Any]:
    phase2fv = _read_json(phase2fv_report_json)
    phase2fu_report_json = phase2fv.get("evidence", {}).get("phase2fu_report_json")
    if not phase2fu_report_json:
        raise ValueError("Phase2FW requires Phase2FV evidence.phase2fu_report_json")
    phase2fu = _read_json(phase2fu_report_json)
    source_manifest_path = Path(
        str(phase2fu.get("evidence", {}).get("reproducibility_manifest", ""))
    )
    if not source_manifest_path.exists():
        raise ValueError("Phase2FW requires a readable Phase2FU manifest")
    ...
    replay_manifest_path = output_root / "phase2fu_replayed_manifest.json"
    _write_json(replay_manifest_path, replay_manifest)
    replay_phase2fu_report = deepcopy(phase2fu)
    replay_phase2fu_report.setdefault("evidence", {})[
        "reproducibility_manifest"
    ] = str(replay_manifest_path)
    replay_phase2fu_report_path = output_root / "phase2fu_replayed_report.json"
    _write_json(replay_phase2fu_report_path, replay_phase2fu_report)
    replay_validation = (
        validate_phase2fu_replay_bundle_summary_reproducibility_manifest(
            replay_phase2fu_report
        )
    )
    replay_validation_path = output_root / "phase2fu_replayed_validation.json"
    ...
    checks = {
        ...
        "replay_phase2fu_report_written": replay_phase2fu_report_path.exists(),
        "replay_phase2fu_validation_passed": replay_validation.get("passed") is True,
    ...
    passed = all(checks.values())
    report = {
        "artifact_family": "phase2fw_replay_bundle_summary_manifest_cross_directory_replay",
        "passed": passed,
        ...
        "source_summary": {
            ...
            "phase2fu_control_artifact_count": phase2fu.get("metrics", {}).get(
        ...
        "supported_claims": [
            (
                "cross-directory replay of the bounded Phase2FU reproducibility "
        ...
        "next_required_experiment": (
            ...
            if passed
            else "repair_phase2fw_replay_bundle_summary_manifest_cross_directory_replay"
        ),
        "evidence": {
            ...
            "replayed_phase2fu_report": str(replay_phase2fu_report_path),
            "replayed_phase2fu_validation": str(replay_validation_path),
        },
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Replay Phase2FU reproducibility manifest in a separate directory."
    )
    parser.add_argument("--phase2fv-report-json", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--output-report-json", required=True)
    parser.add_argument("--no-fail", action="store_true")
    args = parser.parse_args()
    report = audit_phase2fw_replay_bundle_summary_manifest_cross_directory_replay(
        phase2fv_report_json=args.phase2fv_report_json,

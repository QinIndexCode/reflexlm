...
def _materialize_control_report(
    *,
    ...
    if not source_output_dir:
        raise ValueError("Phase2DB requires Phase2DA evidence.composed_grid_output_dir")
...
def _mutate_source_phase2cz_failed(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    report.setdefault("checks", {})["source_phase2cz_passed"] = False
...
def _mutate_perturbation_count_collapsed(
    report: dict[str, Any],
    case_dir: Path,
) -> None:
    del case_dir
    for seed_row in _seed_rows(report):
        seed_row["perturbation_results"] = seed_row.get("perturbation_results", [])[:2]
...
CONTROL_SPECS: tuple[dict[str, Any], ...] = (
    {
        "control_id": "positive_control_original_phase2da_report",
        "mutation": None,
        "expected_passed": True,
        "expected_failed_checks": [],
    },
    {
        "control_id": "negative_source_phase2cz_failed",
        "mutation": _mutate_source_phase2cz_failed,
...
def _run_control(
    *,
    ...
    control_report_json = case_dir / "phase2da_control_report.json"
...
def audit_phase2db_composed_grid_negative_controls(
    *,
    phase2da_report_json: str | Path,
    output_dir: str | Path,
    ...
    negative_rows = [
        ...
        if row["control_id"] != "positive_control_original_phase2da_report"
    ]
    checks = {
        "source_phase2da_passed": phase2da_report.get("passed") is True,
        "positive_control_still_passes": any(
            row["control_id"] == "positive_control_original_phase2da_report"
    ...
    report = {
        "artifact_family": "phase2db_composed_grid_negative_controls",
        "passed": passed,
        "ready_for_phase2da_gate_strictness_claim": passed,
        ...
        "supported_claims": [
            (
                "the Phase2DA composed-grid gate rejects overclaims for source-gate "
                "failure, seed-suite loss or mismatch, collapsed seed or perturbation "
                "coverage, subprocess failures, missing or failed runtime reports, "
                "stress coverage loss, degraded recovery, signature drift, and "
        ...
        "next_required_experiment": (
            ...
            if passed
            else "repair_phase2db_composed_grid_negative_controls"
        ),
        "evidence": {
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Audit Phase2DB composed-grid negative controls."
    )
    parser.add_argument("--phase2da-report-json", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--output-report-json", required=True)
    parser.add_argument("--no-fail", action="store_true")
    args = parser.parse_args()
    report = audit_phase2db_composed_grid_negative_controls(
        phase2da_report_json=args.phase2da_report_json,

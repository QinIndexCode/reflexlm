...
def _phase2es_fixture(tmp_path: Path) -> Path:
    _phase2es_report(tmp_path)
    return tmp_path / "phase2es.json"


def test_phase2et_rejects_manifest_cross_directory_negative_controls(
    tmp_path: Path,
) -> None:
    phase2es = _phase2es_fixture(tmp_path)
    source_report_dir = tmp_path / "es_replay"

...
def _entry_by_role(report: dict[str, Any], key: str, role: str) -> dict[str, Any]:
    for item in _entries(_read_manifest(report), key):
        if item.get("role") == role:
            return item
    raise ValueError(f"missing DW manifest {key} role: {role}")
...
def _copy_entries(entries: list[dict[str, Any]], target_dir: Path) -> list[dict[str, Any]]:
    ...
    for item in entries:
        ...
        if not source.exists():
            raise ValueError(f"Phase2DX source path missing: {source}")
...
def _materialize_control_report(
    *,
    phase2dw_report: dict[str, Any],
    case_dir: Path,
) -> dict[str, Any]:
    control_report = deepcopy(phase2dw_report)
    source_manifest_path = _manifest_path(phase2dw_report)
    if not source_manifest_path.exists():
        raise ValueError("Phase2DX requires a readable Phase2DW manifest")
    ...
    target_manifest_path = manifest_dir / "phase2dw_manifest.json"
...
def _mutate_source_summary_failed(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    report.setdefault("source_summary", {})["phase2du_passed"] = False
...
def _mutate_source_reports_not_list(report: dict[str, Any], case_dir: Path) -> None:
    ...
    manifest["source_reports"] = {"role": "phase2ds_report"}
...
def _mutate_missing_source_report_role(report: dict[str, Any], case_dir: Path) -> None:
    ...
    manifest["source_reports"] = [
        ...
        if item.get("role") != "phase2ds_report"
    ]
    _write_manifest(report, manifest)
...
def _mutate_missing_source_report_file(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    source = Path(str(_entry_by_role(report, "source_reports", "phase2ds_report")["path"]))
    if source.exists():
        source.unlink()
...
def _mutate_tampered_source_report_hash(
    ...
    del case_dir
    source = Path(
        str(_entry_by_role(report, "source_reports", "phase2ds_report")["path"])
    )
...
def _mutate_bundle_artifacts_not_list(report: dict[str, Any], case_dir: Path) -> None:
    del case_dir
    manifest = _read_manifest(report)
    manifest["bundle_artifacts"] = {"role": "replayed_portability_markdown"}
    _write_manifest(report, manifest)
...
def _mutate_missing_bundle_artifact_role(
    ...
    manifest["bundle_artifacts"] = [
        ...
        if item.get("role") != "replayed_portability_markdown"
    ]
    _write_manifest(report, manifest)
...
def _mutate_missing_bundle_artifact_file(
    ...
    del case_dir
    artifact = Path(
        str(_entry_by_role(report, "bundle_artifacts", "replayed_portability_markdown")["path"])
    )
    if artifact.exists():
        artifact.unlink()
...
def _mutate_tampered_bundle_artifact_hash(
    ...
    del case_dir
    artifact = Path(str(_entry_by_role(report, "bundle_artifacts", "bundle_readme")["path"]))
...
CONTROL_SPECS: tuple[dict[str, Any], ...] = (
    {
        "control_id": "positive_control_original_phase2dw_manifest",
        "mutation": None,
    ...
    },
    {
        ...
        "expected_failed_checks": ["source_phase2du_and_phase2dv_passed"],
    },
    {
...
def _run_control(
    *,
    ...
    control_report = _materialize_control_report(
        phase2dw_report=phase2dw_report,
        case_dir=case_dir,
    )
    mutation: Mutation | None = control_spec["mutation"]
    if mutation is not None:
        mutation(control_report, case_dir)
    control_report_json = case_dir / "phase2dw_control_report.json"
    _write_json(control_report_json, control_report)
    validation = validate_phase2dw_replay_bundle_reproducibility_manifest(control_report)
    validation_report_json = case_dir / "phase2dw_validation.json"
...
def audit_phase2dx_replay_bundle_manifest_negative_controls(
    *,
    phase2dw_report_json: str | Path,
    output_dir: str | Path,
    output_report_json: str | Path,
) -> dict[str, Any]:
    phase2dw_report = _read_json(phase2dw_report_json)
    ...
    negative_rows = [
        ...
        if row["control_id"] != "positive_control_original_phase2dw_manifest"
    ]
    checks = {
        "source_phase2dw_passed": phase2dw_report.get("passed") is True,
        "positive_control_still_passes": any(
            row["control_id"] == "positive_control_original_phase2dw_manifest"
    ...
    report = {
        "artifact_family": "phase2dx_replay_bundle_manifest_negative_controls",
        "passed": passed,
        "ready_for_phase2dw_gate_strictness_claim": passed,
        ...
        "supported_claims": [
            (
                "the Phase2DW replay-bundle manifest gate rejects source tampering, "
        ...
        "next_required_experiment": (
            ...
            if passed
            else "repair_phase2dx_replay_bundle_manifest_negative_controls"
        ),
        "evidence": {
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Audit Phase2DX replay bundle manifest negative controls."
    )
    parser.add_argument("--phase2dw-report-json", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--output-report-json", required=True)
    parser.add_argument("--no-fail", action="store_true")
    args = parser.parse_args()
    report = audit_phase2dx_replay_bundle_manifest_negative_controls(
        phase2dw_report_json=args.phase2dw_report_json,

...
def _baseline_accuracy(summary: dict[str, Any], split: str = "val") -> float | None:
...
def audit_phase2av_smoke_postflight(
    *,
    training_summary_json: str | Path,
    launch_gate_json: str | Path,
    ...
    launch_gate = _read_json(launch_gate_json)
    ...
    patch_operation_accuracy = _metric(val_metrics, "patch_operation_accuracy")
    patch_template_accuracy = _metric(val_metrics, "patch_template_slot_accuracy")
    patch_target_file_accuracy = _metric(val_metrics, "patch_target_file_slot_accuracy")
    ...
    pairwise = _dict(summary.get("pairwise_candidate_encoding")).get("val")
    max_valid_candidates = (
        _metric(pairwise, "max_valid_candidates_per_row")
        if isinstance(pairwise, dict)
        else None
    )
    checks = {
        "launch_gate_passed": launch_gate.get("passed") is True
        and launch_gate.get("ready_to_start_phase2av_smoke_training") is True,
        "training_summary_present": bool(summary),
        "open_repair_heads_enabled": summary.get("open_repair_heads_enabled") is True,
        "learned_descriptor_heads_enabled": _dict(
            summary.get("learned_patch_descriptor_heads")
        ).get("enabled")
        is True,
        ...
        "command_slot_val_gate": isinstance(command_slot_accuracy, float)
    ...
    if checks["command_slot_val_gate"] is False:
    ...
    slot_confusion = _dict(_dict(val_metrics.get("slot_confusion")).get("command_slot"))
    if slot_confusion and all(set(preds.keys()) <= {"0"} for preds in slot_confusion.values() if isinstance(preds, dict)):
        failure_modes.append("command_slot_majority_slot0_collapse")
    patch_operation_confusion = _dict(_dict(val_metrics.get("slot_confusion")).get("patch_operation"))
    if patch_operation_confusion and all(set(preds.keys()) <= {"2"} for preds in patch_operation_confusion.values() if isinstance(preds, dict)):
        failure_modes.append("patch_operation_majority_insert_import_collapse")
    return {
        "artifact_family": "phase2av_smoke_postflight",
        "passed": passed,
        "ready_for_phase2av_full_training": False,
        "claim_boundary": (
            "Phase2AV smoke postflight tests small-scale non-sealed descriptor "
            "runtime learning only. A failed postflight blocks full training, "
            "packaging, sealed evaluation, freeform patch generation, production "
            "autonomy, and epoch-making claims."
        ),
        "checks": checks,
        "metrics": {
            ...
            "slot_confusion": val_metrics.get("slot_confusion"),
        ...
        "supported_claims": []
        if not passed
        else ["phase2av_nonsealed_smoke_supports_bounded_descriptor_runtime_learning"],
        "unsupported_claims": [
            "learned_descriptor_runtime_delta" if not passed else "full_training_ready",
        ...
        "inputs": {
            ...
            "launch_gate_json": str(Path(launch_gate_json)),
        },
    }


def main() -> None:
    parser = argparse.ArgumentParser(description="Audit Phase2AV smoke postflight.")
    parser.add_argument("--training-summary-json", required=True)
    parser.add_argument("--launch-gate-json", required=True)
    ...
    report = audit_phase2av_smoke_postflight(
        ...
        launch_gate_json=args.launch_gate_json,

...
def _phase2ds_fixture(tmp_path: Path) -> Path:
    ...
    return tmp_path / "phase2ds.json"


def test_phase2dt_rejects_portability_summary_replay_negative_controls(
    tmp_path: Path,
) -> None:
    phase2ds = _phase2ds_fixture(tmp_path)
    source_report = json.loads(phase2ds.read_text(encoding="utf-8"))
    source_markdown = Path(source_report["evidence"]["replayed_markdown"])
